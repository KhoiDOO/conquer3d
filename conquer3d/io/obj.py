...
geometry files with vertex coordinates, triangle/quad face topologies, and per-vertex colors.
"""
...
def write_quad_obj(
    filepath: str,
    vertices: Union[torch.Tensor, np.ndarray],
    quad_faces: Union[torch.Tensor, np.ndarray]
) -> None:
    """Exports a 3D quad mesh with 4-vertex quad faces to a Wavefront OBJ file.

    Args:
        filepath (str): Target file path string ending with `.obj`.
        vertices (Union[torch.Tensor, np.ndarray]): Vertex coordinates of shape `(V, 3)`.
        quad_faces (Union[torch.Tensor, np.ndarray]): Quad face indices of shape `(F, 4)`.
    """
    if isinstance(vertices, torch.Tensor):
        vertices = vertices.detach().cpu().numpy()
    if isinstance(quad_faces, torch.Tensor):
        quad_faces = quad_faces.detach().cpu().numpy()

    with open(filepath, 'w') as f:
        f.write("# Wavefront OBJ Quad Mesh generated by Conquer3D\n")
        for v in vertices:
...
def write_voxel_obj(
    ...
    """Exports 3D voxel cells (each with 8 corner indices) as a 6-quad per voxel mesh to Wavefront OBJ.
    ...
    write_quad_obj(filepath, vertices, quad_faces)