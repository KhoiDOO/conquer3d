...
namespace pgs_aabb
{
     ...
     */
    __device__ __forceinline__ bool test_pgs_intersect_voxel(const float3 &mean, const float3 &normal,
                                                             const float3 &gs_ab_min, const float3 &gs_ab_max,
...
#pragma unroll
        for (int i = 0; i < 8; i++)
        {
            float3 corner = make_float3((i & 1) ? vx_ab_max.x : vx_ab_min.x, (i & 2) ? vx_ab_max.y : vx_ab_min.y,
     ...
     */
    __device__ __forceinline__ bool compute_pgs_voxel_centroid(const float3 &mean, const float3 &normal,
                                                               const float *covi, const float iso,
                                                               const float3 &vx_ab_min, const float3 &vx_ab_max,
 ...
 */
    __global__ void query_pgs_voxel_pair_intersection_bvh_kernel(
        ...
        const int *__restrict__ object_ids, const float3 *__restrict__ means, const float3 *__restrict__ normals,
        const float *__restrict__ covis, const float3 *__restrict__ gs_aabb_mins,
        const float3 *__restrict__ gs_aabb_maxs, const float *__restrict__ isos, const float iso,
        const bool return_centroids, const bool return_centroid_densities, bool *__restrict__ hit_mask,
        int64_t *__restrict__ out_voxel_ids, int64_t *__restrict__ out_gaus_ids, float3 *__restrict__ centroids,
        float *__restrict__ densities, int64_t *__restrict__ global_counter, const int64_t max_capacity)
    {
        ...
        bvh::traverse(
            ...
            [&](int leaf_idx)
            {
                ...
                bool broad_hit = test_pgs_intersect_voxel(mean, normal, gs_ab_min, gs_ab_max, vx_ab_min, vx_ab_max);
                ...
                if (broad_hit)
                {
                    ...
                    bool narrow_hit = compute_pgs_voxel_centroid(mean, normal, covi, __iso, vx_ab_min, vx_ab_max,
                                                                 return_centroids, centroid);
        ...
        float *__restrict__ densities, int64_t *__restrict__ global_counter, const int64_t max_capacity)
    {
        ...
        if (isos != nullptr)
        {
            query_pgs_voxel_pair_intersection_bvh_kernel<true>
                <<<blocks, threads>>>(num_voxels, num_gaussians, vx_aabb_mins, vx_aabb_maxs, bvh_aabb_mins,
                                      bvh_aabb_maxs, bvh_children, object_ids, means, normals, covis, gs_aabb_mins,
                                      gs_aabb_maxs, isos, iso, return_centroids, return_centroid_densities, hit_mask,
                                      out_voxel_ids, out_gaus_ids, centroids, densities, global_counter, max_capacity);
        }
        else
        {
            query_pgs_voxel_pair_intersection_bvh_kernel<false>
                <<<blocks, threads>>>(num_voxels, num_gaussians, vx_aabb_mins, vx_aabb_maxs, bvh_aabb_mins,
                                      bvh_aabb_maxs, bvh_children, object_ids, means, normals, covis, gs_aabb_mins,
                                      gs_aabb_maxs, isos, iso, return_centroids, return_centroid_densities, hit_mask,
                                      out_voxel_ids, out_gaus_ids, centroids, densities, global_counter, max_capacity);
 ...
 */
    __global__ void query_pgs_edge_intersection_bvh_kernel(
        ...
        const float3 *__restrict__ bvh_aabb_maxs, const int2 *__restrict__ bvh_children,
        const int *__restrict__ object_ids, const float3 *__restrict__ means, const float3 *__restrict__ normals,
        const float *__restrict__ opacities, const float *__restrict__ covis, const float *__restrict__ isos,
        const float iso, bool *__restrict__ hit_mask, int64_t *__restrict__ out_gaus_ids)
    {
        ...
        bvh::traverse(
            ...
            [&](int leaf_idx)
            {
                ...
                bool hit = pgs::test_pgs_segment(mean, normal, covi, __iso, edge_start, edge_end, t_hit);

                if (hit)
                {
                    any_hit = true;

                    float3 p_hit = make_float3(edge_start.x + t_hit * (edge_end.x - edge_start.x),
    ...
    void query_pgs_edge_intersection_bvh(const uint32_t num_edges, const uint32_t num_gaussians,
                                         ...
                                         const float3 *__restrict__ means, const float3 *__restrict__ normals,
                                         ...
                                         int64_t *__restrict__ out_gaus_ids)
    {
        ...
        if (isos != nullptr)
        {
            query_pgs_edge_intersection_bvh_kernel<true><<<blocks, threads>>>(
                ...
                object_ids, means, normals, opacities, covis, isos, iso, hit_mask, out_gaus_ids);
        }
        else
        {
