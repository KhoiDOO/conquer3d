...
struct Ray
{
    ...
    __host__ __device__ __forceinline__ Ray()
        : origin(make_float3(0.0f, 0.0f, 0.0f)), direction(make_float3(0.0f, 0.0f, 1.0f)),
          inv_direction(make_float3(1e30f, 1e30f, 1.0f)), t_min(0.0f), t_max(1e30f)
    {
    }
     ...
     */
    __host__ __device__ __forceinline__ float3 at(float t) const { return origin + t * direction; }
     ...
     */
    __host__ __device__ __forceinline__ bool is_intersect_aabb(const float3 &aabb_min, const float3 &aabb_max,
                                                               float &t_hit) const
    {
