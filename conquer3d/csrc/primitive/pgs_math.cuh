...
namespace pgs
{
     ...
     */
    __device__ __forceinline__ bool test_pgs_segment(const float3 &mean, const float3 &normal, const float *covi,
                                                     const float iso, const float3 &segment_start,
                                                     const float3 &segment_end, float &t_hit)
    {
     ...
     */
    __device__ __forceinline__ bool solve_pgs_pair_tangency_radius(const float3 &mean, const float3 &normal,
                                                                   const float *covi, const float3 &neighbor_mean,
                                                                   const float3 &neighbor_normal, float &out_iso)
    {
        ...
        float3x3 m = make_float3x3(n_i.x, n_i.y, n_i.z, n_j.x, n_j.y, n_j.z, d.x, d.y, d.z);
