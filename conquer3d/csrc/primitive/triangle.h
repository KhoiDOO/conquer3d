...
struct Triangle
{
     ...
     */
    __host__ __device__ __forceinline__ float3 compute_centroid() const { return (v0 + v1 + v2) * (1.0f / 3.0f); }
    ...
    {
        aabb_min =
            make_float3(fminf(v0.x, fminf(v1.x, v2.x)), fminf(v0.y, fminf(v1.y, v2.y)), fminf(v0.z, fminf(v1.z, v2.z)));
        aabb_max =
            make_float3(fmaxf(v0.x, fmaxf(v1.x, v2.x)), fmaxf(v0.y, fmaxf(v1.y, v2.y)), fmaxf(v0.z, fmaxf(v1.z, v2.z)));
    }
    ...
    __host__ __device__ __forceinline__ float3 compute_circumcenter(bool strict_inside = false) const
    {
        ...
        float3 circumcenter =
            v2 + (maths::cross(cross_ab, a) * maths::dot(b, b) + maths::cross(b, cross_ab) * maths::dot(a, a)) *
                     (1.0f / denom);
    ...
    {
        if (!test_point_on_tria_plane(p, eps))
            return false;
    ...
    __host__ __device__ __forceinline__ bool is_voxel_intersect(const float3 &voxel_min, const float3 &voxel_max) const
    {
        ...
        if (!aabb::test_aabb_overlap(voxel_min, voxel_max, tri_min, tri_max))
            return false;
...
#define TEST_AXIS(axis)                                                                                                \
    ...
    {                                                                                                                  \
        ...
        if (fminf(fminf(p0_proj, p1_proj), p2_proj) > r || fmaxf(fmaxf(p0_proj, p1_proj), p2_proj) < -r)               \
            return false;                                                                                              \
    } while (0)
...
namespace triangle
{
     ...
     * @return The circumcentre.
     */
    __host__ __device__ __inline__ float3 compute_circumcenter(const float3 &v0, const float3 &v1, const float3 &v2,
     ...
     * @param[out] aabb_max Upper bound.
     */
    __device__ __inline__ void compute_aabb(const float3 &v0, const float3 &v1, const float3 &v2, float3 &aabb_min,
                                            float3 &aabb_max)
    {
     ...
     * @return True if the triangle meets the box.
     */
    __host__ __device__ __inline__ bool is_voxel_intersect(const float3 &v0, const float3 &v1, const float3 &v2,
                                                           const float3 &voxel_min, const float3 &voxel_max)
    {
     ...
     * @return True if within @p eps of the plane.
     */
    __host__ __device__ __inline__ bool test_point_on_tria_plane(const float3 &v0, const float3 &v1, const float3 &v2,
                                                                 const float3 &p, float eps = 1e-5f)
    {
     ...
     */
    __host__ __device__ __inline__ bool test_point_inside_on_tria_plane(const float3 &v0, const float3 &v1,
                                                                        const float3 &v2, const float3 &p)
    {
     ...
     * @return True if the point lies on the triangle.
     */
    __host__ __device__ __inline__ bool test_point_inside(const float3 &v0, const float3 &v1, const float3 &v2,
                                                          const float3 &p, float eps = 1e-5f)
    {
