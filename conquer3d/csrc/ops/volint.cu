 ...
 *
 * `mode == 1` scales the depth residual by $\|\mathbf{p}_{cam}\| / z_c$ for a true Euclidean
 * distance along the viewing ray (matching Open3D's `UniformTSDFVolume`); any other value
 * keeps the cheaper projective residual $d - z_c$, accurate near the optical axis and
 * increasingly approximate towards the corners. Vertices behind the camera ($z_c \le 0$),
 * outside the image, on invalid depth ($d \le 0$), or behind the truncation band are skipped
 ...
 * @warning Each vertex is written by exactly one thread, so no atomics are needed -- and
 * so integrating several frames concurrently into one volume would race. Call once per
 * frame.
 */
__global__ void single_view_volume_integral_kernel(const int num_vertices, const float3 *grid_vertices, float *sdf,
                                                   float *weight, float3 *color, const float *depth_image,
                                                   const float3 *color_image, const int image_width,
                                                   const int image_height, const float4x4 extrinsics,
                                 ...
                                 const float3x3 intrinsics, const float trunc_margin, const int mode)
{
    ...
    single_view_volume_integral_kernel<<<grid_size, block_size>>>(num_vertices, grid_vertices, sdf, weight, color,
                                                                  depth_image, color_image, image_width, image_height,
                                                                  extrinsics, intrinsics, trunc_margin, mode);
}