        ...
        int num_objects)
    {
        ...
        bvh::traverse(num_objects, bvh_children,
            [&] (int node_idx) {
                ...
                return !(box_max.x < node_min.x || box_min.x > node_max.x ||
            ...
            [&] (int leaf_idx) {
                ...
                if (T.is_voxel_intersect(box_min, box_max)) { hit = true; return false; }
                return true;
            });
        return hit;
    }
...
__global__ void coarse_to_fine_kernel(
        ...
        int* __restrict__ changed_flag)
    {
        ...
        if (fi == 0 && ci > 0 && coarse_mask[(ci - 1) * (CY * CZ) + cj * CZ + ck] == 2) {
            ...
            if (!test_segment_intersect_mesh(pA, pB, bvh_aabb_mins, bvh_aabb_maxs, bvh_children, object_ids, vertices, triangles, num_objects)) seed = true;
        }
