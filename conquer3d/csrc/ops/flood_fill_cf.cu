...
namespace ops
{
     ...
     */
    __global__ void init_coarse_grid_kernel(int8_t *__restrict__ coarse_mask, int CX, int CY, int CZ, int BX, int BY,
                                            int BZ, float3 grid_min, float3 fine_spacing,
                                            ...
                                            int num_objects)
    {
        ...
        float3 box_min = make_float3(grid_min.x + ci * BX * fine_spacing.x, grid_min.y + cj * BY * fine_spacing.y,
                                     grid_min.z + ck * BZ * fine_spacing.z);
        float3 box_max =
            make_float3(grid_min.x + (ci + 1) * BX * fine_spacing.x, grid_min.y + (cj + 1) * BY * fine_spacing.y,
                        grid_min.z + (ck + 1) * BZ * fine_spacing.z);

        bool intersects = test_box_overlap_bvh_cf(box_min, box_max, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
                                                  object_ids, vertices, triangles, num_objects);
     ...
     */
    __global__ void init_perimeter_seeds_kernel(int8_t *__restrict__ coarse_mask, int CX, int CY, int CZ)
    {
        int64_t idx = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        int64_t total_coarse = (int64_t)CX * CY * CZ;
        if (idx >= total_coarse)
     ...
     */
    __global__ void init_boundary_perimeter_faces_kernel(const int3 *__restrict__ boundary_coords,
                                                         int8_t *__restrict__ fine_masks, int num_boundary_blocks,
                                                         int CX, int CY, int CZ, int BX, int BY, int BZ)
    {
        int block_id = blockIdx.x;
        if (block_id >= num_boundary_blocks)
            return;
     ...
     */
    __global__ void coarse_to_coarse_kernel(int8_t *__restrict__ coarse_mask, int CX, int CY, int CZ, int BX, int BY,
                                            int BZ, float3 grid_min, float3 fine_spacing,
                                            const float3 *__restrict__ bvh_aabb_mins,
                                            const float3 *__restrict__ bvh_aabb_maxs,
                                            const int2 *__restrict__ bvh_children, const int *__restrict__ object_ids,
                                            const float3 *__restrict__ vertices, const int3 *__restrict__ triangles,
                                            int num_objects, int *__restrict__ changed_flag)
    {
        int64_t idx = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        int64_t total_coarse = (int64_t)CX * CY * CZ;
        if (idx >= total_coarse)
        ...
        float3 pA =
            make_float3(grid_min.x + (ci + 0.5f) * BX * fine_spacing.x, grid_min.y + (cj + 0.5f) * BY * fine_spacing.y,
        ...
        for (int k = 0; k < 6; ++k)
        {
            ...
            if (ni >= 0 && ni < CX && nj >= 0 && nj < CY && nk >= 0 && nk < CZ)
            {
                ...
                {
                    float3 pB = make_float3(grid_min.x + (ni + 0.5f) * BX * fine_spacing.x,
                                            grid_min.y + (nj + 0.5f) * BY * fine_spacing.y,
                                            grid_min.z + (nk + 0.5f) * BZ * fine_spacing.z);
                    if (!test_segment_intersect_mesh(pA, pB, bvh_aabb_mins, bvh_aabb_maxs, bvh_children, object_ids,
     ...
     * another sweep is required; the fill has converged once a full round leaves it clear.
     */
    __global__ void coarse_to_fine_kernel(const int3 *__restrict__ boundary_coords,
                                          const int8_t *__restrict__ coarse_mask, int8_t *__restrict__ fine_masks,
                                          int num_boundary_blocks, int CX, int CY, int CZ, int BX, int BY, int BZ,
                                          float3 grid_min, float3 fine_spacing,
                                          const float3 *__restrict__ bvh_aabb_mins,
                                          const float3 *__restrict__ bvh_aabb_maxs,
                                          const int2 *__restrict__ bvh_children, const int *__restrict__ object_ids,
                                          const float3 *__restrict__ vertices, const int3 *__restrict__ triangles,
                                          int num_objects, int *__restrict__ changed_flag)
    {
        ...
        float3 pA =
            make_float3(grid_min.x + (ci * BX + fi) * fine_spacing.x, grid_min.y + (cj * BY + fj) * fine_spacing.y,
        ...
        if (fi == 0 && ci > 0 && coarse_mask[(ci - 1) * (CY * CZ) + cj * CZ + ck] == 2)
        {
            ...
            if (!test_segment_intersect_mesh(pA, pB, bvh_aabb_mins, bvh_aabb_maxs, bvh_children, object_ids, vertices,
                                             triangles, num_objects))
                seed = true;
        }
     ...
     */
    __global__ void
    fine_intra_block_bfs_kernel(const int3 *__restrict__ boundary_coords, int8_t *__restrict__ fine_masks,
                                int num_boundary_blocks, int BX, int BY, int BZ, float3 grid_min, float3 fine_spacing,
            ...
            if (s_mask[fi][fj][fk] == 2)
            {
                for (int k = 0; k < 6; ++k)
                {
                    ...
                    if (nfi >= 0 && nfi < BX && nfj >= 0 && nfj < BY && nfk >= 0 && nfk < BZ)
                    {
                        ...
                        {
                            float3 pB = make_float3(grid_min.x + (ci * BX + nfi) * fine_spacing.x,
                                                    grid_min.y + (cj * BY + nfj) * fine_spacing.y,
                                                    grid_min.z + (ck * BZ + nfk) * fine_spacing.z);

                            if (!test_segment_intersect_mesh(pA, pB, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
                                                             object_ids, vertices, triangles, num_objects))
                            {
     ...
     */
    __global__ void
    fine_to_fine_inter_block_kernel(const int3 *__restrict__ boundary_coords,
                                    const int32_t *__restrict__ boundary_lookup, int8_t *__restrict__ fine_masks,
                                    int num_boundary_blocks, int CX, int CY, int CZ, int BX, int BY, int BZ,
                                    float3 grid_min, float3 fine_spacing, const float3 *__restrict__ bvh_aabb_mins,
                                    const float3 *__restrict__ bvh_aabb_maxs, const int2 *__restrict__ bvh_children,
                                    const int *__restrict__ object_ids, const float3 *__restrict__ vertices,
                                    const int3 *__restrict__ triangles, int num_objects, int *__restrict__ changed_flag)
    {
        int block_id = blockIdx.x;
        if (block_id >= num_boundary_blocks)
            return;
            ...
            if (nci != ci || ncj != cj || nck != ck)
            {
                if (nci >= 0 && nci < CX && ncj >= 0 && ncj < CY && nck >= 0 && nck < CZ)
                {
                    ...
                    if (neighbor_block_id >= 0)
                    {
                        ...
                        {
                            float3 pB = make_float3(grid_min.x + (nci * BX + nfi) * fine_spacing.x,
                                                    grid_min.y + (ncj * BY + nfj) * fine_spacing.y,
                                                    grid_min.z + (nck * BZ + nfk) * fine_spacing.z);
                            if (!test_segment_intersect_mesh(pA, pB, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
                                                             object_ids, vertices, triangles, num_objects))
                            {
                                if (atomicCAS_int8(&my_fine_mask[local_idx], -2, 2) == -2)
                                {
                                    atomicExch(changed_flag, 1);
                                }
                            }
     ...
     * another sweep is required; the fill has converged once a full round leaves it clear.
     */
    __global__ void fine_to_coarse_kernel(const int3 *__restrict__ boundary_coords,
                                          const int8_t *__restrict__ fine_masks, int8_t *__restrict__ coarse_mask,
                                          int num_boundary_blocks, int CX, int CY, int CZ, int BX, int BY, int BZ,
                                          float3 grid_min, float3 fine_spacing,
                                          const float3 *__restrict__ bvh_aabb_mins,
                                          const float3 *__restrict__ bvh_aabb_maxs,
                                            ...
                                            int connectivity)
    {
        ...
        int64_t bx = (block_size.size() >= 3 && block_size[0] > 0) ? block_size[0]
                                                                   : choose_best_divisor(static_cast<int>(rx), 8);
        int64_t by = (block_size.size() >= 3 && block_size[1] > 0) ? block_size[1]
                                                                   : choose_best_divisor(static_cast<int>(ry), 8);
        int64_t bz = (block_size.size() >= 3 && block_size[2] > 0) ? block_size[2]
                                                                   : choose_best_divisor(static_cast<int>(rz), 8);
        ...
        float3 f_spacing = make_float3((rx > 1) ? (grid_max[0] - grid_min[0]) / (rx - 1) : 1.0f,
        ...
        init_coarse_grid_kernel<<<blocks, threads>>>(
            coarse_mask.data_ptr<int8_t>(), cx, cy, cz, bx, by, bz, f_min, f_spacing,
            (const float3 *)aabb_mins.data_ptr<float>(), (const float3 *)aabb_maxs.data_ptr<float>(),
            (const int2 *)bvh_children.data_ptr<int>(), object_ids.data_ptr<int>(),
        ...
        init_perimeter_seeds_kernel<<<blocks, threads>>>(coarse_mask.data_ptr<int8_t>(), cx, cy, cz);

        if (num_boundary_blocks > 0)
        {
            dim3 fine_block(bx, by, bz);
            init_boundary_perimeter_faces_kernel<<<static_cast<int>(num_boundary_blocks), fine_block>>>(
                (const int3 *)boundary_coords.data_ptr<int>(), fine_boundary_masks.data_ptr<int8_t>(),
                static_cast<int>(num_boundary_blocks), cx, cy, cz, bx, by, bz);
        }
        ...
        for (int iter = 0; iter < 100; ++iter)
        {
            ...
            coarse_to_coarse_kernel<<<blocks, threads>>>(
                ...
                (const int2 *)bvh_children.data_ptr<int>(), object_ids.data_ptr<int>(),
                (const float3 *)vertices.data_ptr<float>(), (const int3 *)triangles.data_ptr<int>(),
                static_cast<int>(object_ids.size(0)), changed_flag.data_ptr<int>());

            if (num_boundary_blocks > 0)
            {
                // 4b. Coarse-to-Fine Seeding
                coarse_to_fine_kernel<<<static_cast<int>(num_boundary_blocks), fine_block>>>(
                    (const int3 *)boundary_coords.data_ptr<int>(), coarse_mask.data_ptr<int8_t>(),
                    fine_boundary_masks.data_ptr<int8_t>(), static_cast<int>(num_boundary_blocks), cx, cy, cz, bx, by,
                    bz, f_min, f_spacing, (const float3 *)aabb_mins.data_ptr<float>(),
                    (const float3 *)aabb_maxs.data_ptr<float>(), (const int2 *)bvh_children.data_ptr<int>(),
                    object_ids.data_ptr<int>(), (const float3 *)vertices.data_ptr<float>(),
                    (const int3 *)triangles.data_ptr<int>(), static_cast<int>(object_ids.size(0)),
                ...
                fine_intra_block_bfs_kernel<<<static_cast<int>(num_boundary_blocks), fine_block>>>(
                    (const int3 *)boundary_coords.data_ptr<int>(), fine_boundary_masks.data_ptr<int8_t>(),
                    static_cast<int>(num_boundary_blocks), bx, by, bz, f_min, f_spacing,
                    (const float3 *)aabb_mins.data_ptr<float>(), (const float3 *)aabb_maxs.data_ptr<float>(),
                    (const int2 *)bvh_children.data_ptr<int>(), object_ids.data_ptr<int>(),
                    (const float3 *)vertices.data_ptr<float>(), (const int3 *)triangles.data_ptr<int>(),
                    static_cast<int>(object_ids.size(0)), changed_flag.data_ptr<int>());
                ...
                fine_to_fine_inter_block_kernel<<<static_cast<int>(num_boundary_blocks), fine_block>>>(
                    (const int3 *)boundary_coords.data_ptr<int>(), boundary_block_lookup.data_ptr<int32_t>(),
                    fine_boundary_masks.data_ptr<int8_t>(), static_cast<int>(num_boundary_blocks), cx, cy, cz, bx, by,
                    bz, f_min, f_spacing, (const float3 *)aabb_mins.data_ptr<float>(),
                    (const float3 *)aabb_maxs.data_ptr<float>(), (const int2 *)bvh_children.data_ptr<int>(),
                    object_ids.data_ptr<int>(), (const float3 *)vertices.data_ptr<float>(),
                ...
                fine_to_coarse_kernel<<<static_cast<int>(num_boundary_blocks), fine_block>>>(
                    (const int3 *)boundary_coords.data_ptr<int>(), fine_boundary_masks.data_ptr<int8_t>(),
                    coarse_mask.data_ptr<int8_t>(), static_cast<int>(num_boundary_blocks), cx, cy, cz, bx, by, bz,
                    f_min, f_spacing, (const float3 *)aabb_mins.data_ptr<float>(),
                    (const float3 *)aabb_maxs.data_ptr<float>(), (const int2 *)bvh_children.data_ptr<int>(),
                    object_ids.data_ptr<int>(), (const float3 *)vertices.data_ptr<float>(),
                    (const int3 *)triangles.data_ptr<int>(), static_cast<int>(object_ids.size(0)),
                    changed_flag.data_ptr<int>());
