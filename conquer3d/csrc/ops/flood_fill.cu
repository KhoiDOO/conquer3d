...
namespace ops
{
     ...
     */
    __global__ void init_perimeter_kernel(int8_t *__restrict__ mask, int *__restrict__ frontier,
                                          int *__restrict__ frontier_size, int RX, int RY, int RZ)
    {
     ...
     */
    __global__ void
    flood_fill_step_kernel(int8_t *__restrict__ mask, const int *__restrict__ current_frontier, int frontier_size,
                           int *__restrict__ next_frontier, int *__restrict__ next_frontier_size, int RX, int RY,
                           int RZ, int connectivity, float3 grid_min, float3 grid_spacing,
                           ...
                           const float3 *__restrict__ vertices, const int3 *__restrict__ triangles, int num_objects)
    {
        ...
        float3 pA = make_float3(grid_min.x + vi * grid_spacing.x, grid_min.y + vj * grid_spacing.y,
        ...
        for (int di = -1; di <= 1; di++)
        {
            for (int dj = -1; dj <= 1; dj++)
            {
                for (int dk = -1; dk <= 1; dk++)
                {
                    ...
                    if (ni < 0 || ni >= RX || nj < 0 || nj >= RY || nk < 0 || nk >= RZ)
                        continue;

                    int n_idx = ni * (RY * RZ) + nj * RZ + nk;
                    if (mask[n_idx] == -2)
                    {
                        float3 pB = make_float3(grid_min.x + ni * grid_spacing.x, grid_min.y + nj * grid_spacing.y,
                                     ...
                                     std::vector<int64_t> grid_res, int connectivity)
    {
        ...
        init_perimeter_kernel<<<blocks, threads>>>(mask.data_ptr<int8_t>(), current_frontier.data_ptr<int>(),
                                                   frontier_size.data_ptr<int>(), static_cast<int>(rx),
                                                   static_cast<int>(ry), static_cast<int>(rz));
        ...
        while (curr_size > 0)
        {
            ...
            flood_fill_step_kernel<<<step_blocks, threads>>>(
                mask.data_ptr<int8_t>(), current_frontier.data_ptr<int>(), curr_size, next_frontier.data_ptr<int>(),
                next_frontier_size.data_ptr<int>(), static_cast<int>(rx), static_cast<int>(ry), static_cast<int>(rz),
                connectivity, f_min, f_spacing, (const float3 *)aabb_mins.data_ptr<float>(),
