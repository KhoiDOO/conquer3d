...
namespace mca
{
     ...
     */
    __device__ __forceinline__ void evaluate_face_segments(int f, const float F[8], float iso, uint8_t next_e[12])
    {
        int v0 = v_face[f][0];
        int v1 = v_face[f][1];
        int v2 = v_face[f][2];
     ...
     */
    __device__ __forceinline__ int trace_mca_loops(const float F[8], float iso, uint8_t out_loops[4][12],
...
#pragma unroll
     ...
     */
    __global__ void compute_active_voxels_mca_kernel(const uint32_t num_voxels, const uint32_t *__restrict__ voxels,
                                                     ...
                                                     uint32_t *__restrict__ triangle_counts)
    {
        ...
        float F[8] = {sdf[v0], sdf[v1], sdf[v2], sdf[v3], sdf[v4], sdf[v5], sdf[v6], sdf[v7]};
...
#pragma unroll
    ...
    __global__ void generate_edges_and_triangles_mca_kernel(
        const uint32_t num_voxels, const uint32_t *__restrict__ voxels, const float *__restrict__ sdf, const float iso,
        const uint32_t *__restrict__ tri_offsets, uint64_t *__restrict__ out_edges, int3 *__restrict__ out_triangles)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_voxels)
            return;
        ...
        uint32_t v[8] = {voxels[idx * 8 + 0], voxels[idx * 8 + 1], voxels[idx * 8 + 2], voxels[idx * 8 + 3],
                         voxels[idx * 8 + 4], voxels[idx * 8 + 5], voxels[idx * 8 + 6], voxels[idx * 8 + 7]};

        float F[8] = {sdf[v[0]], sdf[v[1]], sdf[v[2]], sdf[v[3]], sdf[v[4]], sdf[v[5]], sdf[v[6]], sdf[v[7]]};
...
#pragma unroll
        ...
        if (ambig_mask == 0)
        {
            ...
            {
                int local_edges[3] = {tri_edges[t * 3 + 0], tri_edges[t * 3 + 2], tri_edges[t * 3 + 1]};
#pragma unroll
...
#pragma unroll
     ...
     */
    __global__ void remap_triangles_kernel(const uint32_t num_triangles, const uint32_t *__restrict__ edge_indices,
                                           int3 *__restrict__ triangles)
    {
        ...
        triangles[idx] = make_int3(edge_indices[idx * 3 + 0], edge_indices[idx * 3 + 1], edge_indices[idx * 3 + 2]);
    }
     ...
     */
    __global__ void
    interpolate_vertices_and_features_kernel(const uint32_t num_edges, const uint64_t *__restrict__ unique_edge_keys,
                                             const float3 *__restrict__ grid_vertices, const float *__restrict__ sdf,
                                             const float *__restrict__ in_colors, const int num_color_channels,
                                             const float iso, float3 *__restrict__ out_vertices,
     ...
     * varies between runs.
     */
    __global__ void backward_dmca_kernel(const uint32_t num_edges, const uint64_t *__restrict__ unique_edge_keys,
                                         const float3 *__restrict__ grad_vertices,
                                         const float *__restrict__ grad_colors,
                                         const float3 *__restrict__ grid_vertices, const float *__restrict__ in_colors,
                                         const int num_color_channels, const float *__restrict__ sdf, const float iso,
                                         float *__restrict__ grad_sdf, float *__restrict__ grad_in_colors)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_edges)
            return;
    ...
    marching_cubes_asymptotic(const torch::Tensor &grid_vertices, const torch::Tensor &voxels, const torch::Tensor &sdf,
                              const std::optional<torch::Tensor> &colors, float iso)
    {
        ...
        compute_active_voxels_mca_kernel<<<num_blocks, block_size>>>(
            num_voxels, reinterpret_cast<const uint32_t *>(voxels.data_ptr<int32_t>()), sdf.data_ptr<float>(), iso,
        ...
        cudaMemcpy(&total_triangles, tri_offsets.data_ptr<int32_t>() + num_voxels - 1, sizeof(uint32_t),
                   cudaMemcpyDeviceToHost);
        uint32_t last_count = 0;
        cudaMemcpy(&last_count, tri_counts.data_ptr<int32_t>() + num_voxels - 1, sizeof(uint32_t),
                   cudaMemcpyDeviceToHost);
        total_triangles += last_count;

        if (total_triangles == 0)
        {
            return std::make_tuple(torch::empty({0, 3}, options_f32), torch::empty({0, 3}, options_i32),
                                   colors.has_value()
                                       ? std::make_optional(torch::empty({0, colors.value().size(1)}, options_f32))
                                       : std::nullopt);
        }
        ...
        auto d_out_indices =
            thrust::device_pointer_cast(reinterpret_cast<uint32_t *>(edge_indices.data_ptr<int32_t>()));
        auto d_uniq = thrust::device_pointer_cast(reinterpret_cast<uint64_t *>(unique_edges.data_ptr<int64_t>()));

        thrust::lower_bound(policy, d_uniq, d_uniq + num_unique_edges, d_raw, d_raw + total_triangles * 3,
                            d_out_indices);

        int tri_blocks = (total_triangles + block_size - 1) / block_size;
        remap_triangles_kernel<<<tri_blocks, block_size>>>(
            total_triangles, reinterpret_cast<const uint32_t *>(edge_indices.data_ptr<int32_t>()),
            reinterpret_cast<int3 *>(triangles.data_ptr<int32_t>()));
        ...
        interpolate_vertices_and_features_kernel<<<edge_blocks, block_size>>>(
            num_unique_edges, reinterpret_cast<const uint64_t *>(unique_edges.data_ptr<int64_t>()),
            reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), sdf.data_ptr<float>(), in_colors_ptr,
            num_color_channels, iso, reinterpret_cast<float3 *>(out_vertices.data_ptr<float>()), out_colors_ptr);

        return std::make_tuple(out_vertices, triangles, out_colors);
    }

    std::tuple<torch::Tensor, std::optional<torch::Tensor>>
    marching_cubes_asymptotic_backward(const torch::Tensor &grad_vertices,
                                       const std::optional<torch::Tensor> &grad_colors,
                                       const torch::Tensor &grid_vertices, const torch::Tensor &unique_edges,
                                       const torch::Tensor &sdf, const std::optional<torch::Tensor> &colors, float iso)
    {
        ...
        if (num_edges > 0)
        {
            ...
            backward_dmca_kernel<<<num_blocks, block_size>>>(
                num_edges, reinterpret_cast<const uint64_t *>(unique_edges.data_ptr<int64_t>()),
                reinterpret_cast<const float3 *>(grad_vertices.data_ptr<float>()), grad_colors_ptr,
                reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), in_colors_ptr, num_color_channels,
                sdf.data_ptr<float>(), iso, grad_sdf.data_ptr<float>(), grad_in_colors_ptr);
        }
