...
namespace conquer3d
{
    namespace ops
    {
        ...
        namespace
        {
             ...
             */
            __device__ __forceinline__ float3 compute_trilinear_normal(float u, float v, float w, const float s[8],
             ...
             */
            __device__ __forceinline__ float triangle_min_angle(const float3 &p0, const float3 &p1, const float3 &p2)
            {
                float3 e0 = p1 - p0;
                float3 e1 = p2 - p1;
                float3 e2 = p0 - p2;
         ...
         */
        __global__ void
        compute_dual_vertices_kernel(const float3 *__restrict__ grid_vertices, const int *__restrict__ voxels,
                                     const float *__restrict__ sdf, const float3 *__restrict__ grid_normals,
                                     const float3 *__restrict__ edge_points, const float3 *__restrict__ edge_normals,
                                     float iso, int num_voxels, float3 *__restrict__ dual_vertices,
                                     int *__restrict__ voxel_is_active, int *__restrict__ bipolar_edge_counts)
        {
...
#pragma unroll
         ...
         */
        __global__ void emit_bipolar_edges_kernel(const int *__restrict__ voxels, const float *__restrict__ sdf,
                                                  const int *__restrict__ edge_offsets, float iso, int num_voxels,
...
#pragma unroll
         ...
         */
        __global__ void gather_dual_quads_kernel(const uint64_t *__restrict__ sorted_edge_keys,
                                                 ...
                                                 const int *__restrict__ voxel_to_compact_idx, int total_instances,
                                                 int4 *__restrict__ out_quads, int *__restrict__ out_quad_count)
        {
...
#pragma unroll
         ...
         * runs. Geometry is unaffected, but a byte-identical mesh is not guaranteed.
         */
        __global__ void quad_to_triangle_kernel(const int4 *__restrict__ quads,
                                                const float3 *__restrict__ compact_vertices, int num_quads,
                                                int3 *__restrict__ out_triangles, int *__restrict__ out_tri_count)
        {
        ...
        __global__ void compact_dual_vertices_and_colors_kernel(
            const float3 *__restrict__ dual_vertices, const int *__restrict__ voxel_is_active,
            const int *__restrict__ voxel_to_compact_idx, const int *__restrict__ voxels,
            const float *__restrict__ colors, const float3 *__restrict__ grid_vertices, int num_voxels,
            int num_channels, float3 *__restrict__ compact_vertices, float *__restrict__ compact_colors)
        {
            int m = blockIdx.x * blockDim.x + threadIdx.x;
            if (m >= num_voxels)
                return;

            if (!voxel_is_active[m])
                return;
...
#pragma unroll
                for (int ch = 0; ch < num_channels; ++ch)
                {
                    ...
                    float c_interp = (1 - u) * (1 - val_v) * (1 - w) * c0 + u * (1 - val_v) * (1 - w) * c1 +
                                     u * val_v * (1 - w) * c2 + (1 - u) * val_v * (1 - w) * c3 +
                                     (1 - u) * (1 - val_v) * w * c4 + u * (1 - val_v) * w * c5 + u * val_v * w * c6 +
                                     (1 - u) * val_v * w * c7;
        ...
        __global__ void backward_dual_contouring_kernel(
            const float3 *__restrict__ grad_verts, const float *__restrict__ grad_colors,
            const float3 *__restrict__ grid_vertices, const int *__restrict__ voxels, const float *__restrict__ sdf,
            const int *__restrict__ voxel_is_active, const int *__restrict__ voxel_to_compact_idx, float iso,
            int num_voxels, int num_channels, float *__restrict__ grad_sdf, float *__restrict__ grad_colors_in)
        {
            int m = blockIdx.x * blockDim.x + threadIdx.x;
            if (m >= num_voxels)
                return;
...
#pragma unroll
         ...
         * @param[out] bipolar_edge_counts Device array of per-voxel bipolar edge counts.
         */
        __global__ void detect_active_voxels_kernel(const int *__restrict__ voxels, const float *__restrict__ sdf,
                                                    float iso, int num_voxels, int *__restrict__ voxel_is_active,
...
#pragma unroll
        ...
        dual_contouring(const at::Tensor &grid_vertices, const at::Tensor &voxels, const at::Tensor &sdf,
                        const c10::optional<at::Tensor> &grid_normals, const c10::optional<at::Tensor> &colors,
                        const c10::optional<at::Tensor> &voxel_vertices, float iso, bool quad_split,
                        const c10::optional<at::Tensor> &edge_points, const c10::optional<at::Tensor> &edge_normals)
        {
            ...
            if (num_voxels == 0)
            {
                ...
                return {at::zeros({0, 3}, grid_vertices.options()), at::zeros({0, face_dim}, voxels.options()),
                        colors.has_value() ? c10::optional<at::Tensor>(
                                                 at::zeros({0, colors.value().size(1)}, colors.value().options()))
                                           : c10::nullopt};
            }
            ...
            if (voxel_vertices.has_value() && voxel_vertices.value().defined() && voxel_vertices.value().numel() > 0)
            {
                ...
                detect_active_voxels_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                    voxels.data_ptr<int>(), sdf.data_ptr<float>(), iso, num_voxels, voxel_is_active.data_ptr<int>(),
            ...
            else
            {
                ...
                const float3 *normals_ptr =
                    grid_normals.has_value() ? reinterpret_cast<const float3 *>(grid_normals.value().data_ptr<float>())
                                             : nullptr;
                const float3 *ep_ptr =
                    (edge_points.has_value() && edge_points.value().defined() && edge_points.value().numel() > 0)
                        ? reinterpret_cast<const float3 *>(edge_points.value().data_ptr<float>())
                        : nullptr;
                const float3 *en_ptr =
                    (edge_normals.has_value() && edge_normals.value().defined() && edge_normals.value().numel() > 0)
                        ? reinterpret_cast<const float3 *>(edge_normals.value().data_ptr<float>())
                        : nullptr;

                compute_dual_vertices_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                    reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), voxels.data_ptr<int>(),
                    sdf.data_ptr<float>(), normals_ptr, ep_ptr, en_ptr, iso, num_voxels,
                    reinterpret_cast<float3 *>(dual_vertices.data_ptr<float>()), voxel_is_active.data_ptr<int>(),
                    bipolar_edge_counts.data_ptr<int>());
            }

            // 2. Prefix sum for active voxels
            at::Tensor voxel_to_compact_idx = at::empty({num_voxels}, voxels.options());
            thrust::exclusive_scan(policy, voxel_is_active.data_ptr<int>(),
                                   voxel_is_active.data_ptr<int>() + num_voxels, voxel_to_compact_idx.data_ptr<int>(),
                                   0);

            int total_active_voxels = 0;
            int last_active = 0;
            int last_compact = 0;
            cudaMemcpyAsync(&last_active, voxel_is_active.data_ptr<int>() + num_voxels - 1, sizeof(int),
                            cudaMemcpyDeviceToHost, at::cuda::getCurrentCUDAStream());
            cudaMemcpyAsync(&last_compact, voxel_to_compact_idx.data_ptr<int>() + num_voxels - 1, sizeof(int),
                            cudaMemcpyDeviceToHost, at::cuda::getCurrentCUDAStream());
            ...
            compact_dual_vertices_and_colors_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                source_dual_vertices_ptr, voxel_is_active.data_ptr<int>(), voxel_to_compact_idx.data_ptr<int>(),
                voxels.data_ptr<int>(), colors_ptr, reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()),
                num_voxels, num_channels, reinterpret_cast<float3 *>(compact_vertices.data_ptr<float>()),
                compact_colors_ptr);

            // 4. Pass 2: Prefix sum for bipolar edge instances
            at::Tensor edge_offsets = at::empty({num_voxels}, voxels.options());
            thrust::exclusive_scan(policy, bipolar_edge_counts.data_ptr<int>(),
                                   bipolar_edge_counts.data_ptr<int>() + num_voxels, edge_offsets.data_ptr<int>(), 0);

            int total_edge_instances = 0;
            int last_edge_count = 0;
            int last_edge_offset = 0;
            cudaMemcpyAsync(&last_edge_count, bipolar_edge_counts.data_ptr<int>() + num_voxels - 1, sizeof(int),
                            cudaMemcpyDeviceToHost, at::cuda::getCurrentCUDAStream());
            cudaMemcpyAsync(&last_edge_offset, edge_offsets.data_ptr<int>() + num_voxels - 1, sizeof(int),
                            cudaMemcpyDeviceToHost, at::cuda::getCurrentCUDAStream());
            cudaStreamSynchronize(at::cuda::getCurrentCUDAStream());
            ...
            {
                int face_dim = quad_split ? 3 : 4;
                return {compact_vertices, at::zeros({0, face_dim}, voxels.options()), compact_colors};
            }
            ...
            emit_bipolar_edges_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                voxels.data_ptr<int>(), sdf.data_ptr<float>(), edge_offsets.data_ptr<int>(), iso, num_voxels,
            ...
            thrust::sort_by_key(policy, reinterpret_cast<uint64_t *>(edge_keys.data_ptr<int64_t>()),
            ...
            gather_dual_quads_kernel<<<edge_blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                ...
                reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), sdf.data_ptr<float>(),
                source_dual_vertices_ptr, voxel_to_compact_idx.data_ptr<int>(), total_edge_instances,
                reinterpret_cast<int4 *>(raw_quads.data_ptr<int>()), quad_count_tensor.data_ptr<int>());

            int num_quads = 0;
            cudaMemcpyAsync(&num_quads, quad_count_tensor.data_ptr<int>(), sizeof(int), cudaMemcpyDeviceToHost,
            ...
            if (quad_split)
            {
                ...
                if (num_quads > 0)
                {
                    ...
                    quad_to_triangle_kernel<<<quad_blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                        ...
                        reinterpret_cast<const float3 *>(compact_vertices.data_ptr<float>()), num_quads,
                        reinterpret_cast<int3 *>(raw_triangles.data_ptr<int>()), tri_count_tensor.data_ptr<int>());
                }
                int num_triangles = 0;
                cudaMemcpyAsync(&num_triangles, tri_count_tensor.data_ptr<int>(), sizeof(int), cudaMemcpyDeviceToHost,
                                at::cuda::getCurrentCUDAStream());
                cudaStreamSynchronize(at::cuda::getCurrentCUDAStream());
        ...
        dual_contouring_backward(const at::Tensor &grad_verts, const c10::optional<at::Tensor> &grad_colors,
                                 const at::Tensor &grid_vertices, const at::Tensor &voxels, const at::Tensor &sdf,
                                 const c10::optional<at::Tensor> &grid_normals, const c10::optional<at::Tensor> &colors,
                                 float iso)
        {
            at::cuda::CUDAGuard device_guard(grid_vertices.device());
            ...
            int threads = 256;
            int blocks = (num_voxels + threads - 1) / threads;
            const float3 *normals_ptr = grid_normals.has_value()
                                            ? reinterpret_cast<const float3 *>(grid_normals.value().data_ptr<float>())
                                            : nullptr;

            compute_dual_vertices_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), voxels.data_ptr<int>(),
                sdf.data_ptr<float>(), normals_ptr, nullptr, nullptr, iso, num_voxels,
                reinterpret_cast<float3 *>(dual_vertices.data_ptr<float>()), voxel_is_active.data_ptr<int>(),
                bipolar_edge_counts.data_ptr<int>());
            ...
            backward_dual_contouring_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                reinterpret_cast<const float3 *>(grad_verts.data_ptr<float>()), grad_colors_ptr,
                reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), voxels.data_ptr<int>(),
                sdf.data_ptr<float>(), voxel_is_active.data_ptr<int>(), voxel_to_compact_idx.data_ptr<int>(), iso,
                num_voxels, num_channels, grad_sdf.data_ptr<float>(), grad_colors_in_ptr);
