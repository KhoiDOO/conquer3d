...
namespace conquer3d
{
    namespace ops
    {
        ...
        namespace
        {
             ...
             */
            __device__ __forceinline__ float trilinear_val(float u, float v, float w, const float s[8])
            {
             ...
             */
            __device__ __forceinline__ float3 trilinear_grad(float u, float v, float w, const float s[8])
            {
             ...
             */
            __device__ inline int extract_cell_contours(float iso, int i_case, const float s[8], int contour_sizes[4],
         ...
         */
        __global__ void dmc_count_contours_kernel(const int *__restrict__ voxels, const float *__restrict__ sdf,
                                                  float iso, int num_voxels, int *__restrict__ contour_counts,
...
#pragma unroll
        ...
        __global__ void dmc_extract_dual_vertices_and_edges_kernel(
            ...
            const float *__restrict__ colors, const float3 *__restrict__ precomputed_vertices,
            const float3 *__restrict__ edge_points, const float3 *__restrict__ edge_normals, int num_channels,
            const int *__restrict__ vert_offsets, const int *__restrict__ edge_offsets, float iso, int project_iters,
            int num_voxels, float3 *__restrict__ out_vertices, float *__restrict__ out_colors,
            uint64_t *__restrict__ out_edge_keys, uint32_t *__restrict__ out_dual_vert_and_edge)
        {
            int m = blockIdx.x * blockDim.x + threadIdx.x;
            if (m >= num_voxels)
                return;
...
#pragma unroll
            ...
            for (int c = 0; c < num_cnt; ++c)
            {
                ...
                else
                {
                    ...
                    float3 pt = make_float3(c_min.x + u * dx, c_min.y + v * dy, c_min.z + w * dz);
                    out_vertices[dual_vert_idx] = pt;
                }
...
#pragma unroll
         ...
         */
        __global__ void dmc_gather_quads_kernel(const uint64_t *__restrict__ sorted_edge_keys,
                                                const uint32_t *__restrict__ sorted_dual_vert_and_edge,
                                                const float *__restrict__ sdf, int total_instances,
...
#pragma unroll
         ...
         * runs. Geometry is unaffected, but a byte-identical mesh is not guaranteed.
         */
        __global__ void dmc_quad_to_triangle_kernel(const int4 *__restrict__ quads, const float3 *__restrict__ vertices,
                                                    int num_quads, int3 *__restrict__ out_triangles,
         ...
         */
        __global__ void dmc_backward_kernel(const float3 *__restrict__ grad_vertices,
                                            const float *__restrict__ grad_colors,
                                            const float3 *__restrict__ grid_vertices, const int *__restrict__ voxels,
                                            const float *__restrict__ sdf, const float *__restrict__ colors,
                                            int num_channels, const int *__restrict__ vert_offsets, float iso,
                                            int num_voxels, float *__restrict__ grad_sdf,
...
#pragma unroll
        ...
        dual_marching_cubes(const at::Tensor &grid_vertices, const at::Tensor &voxels, const at::Tensor &sdf,
                            const c10::optional<at::Tensor> &colors, const c10::optional<at::Tensor> &voxel_vertices,
                            float iso, bool quad_split, int project_iters, const c10::optional<at::Tensor> &edge_points,
                            const c10::optional<at::Tensor> &edge_normals)
        {
            ...
            if (num_voxels == 0)
            {
                ...
                return std::make_tuple(
                    at::empty({0, 3}, grid_vertices.options()), at::empty({0, face_dim}, voxels.options()),
                    colors.has_value() ? c10::make_optional(at::empty({0, colors->size(1)}, colors->options()))
                                       : c10::nullopt);
            }
            ...
            dmc_count_contours_kernel<<<grid, block, 0, stream>>>(voxels.data_ptr<int>(), sdf.data_ptr<float>(), iso,
                                                                  num_voxels, contour_counts.data_ptr<int>(),
            ...
            thrust::exclusive_scan(policy, contour_counts.data_ptr<int>(), contour_counts.data_ptr<int>() + num_voxels,
                                   vert_offsets.data_ptr<int>());
            thrust::exclusive_scan(policy, edge_counts.data_ptr<int>(), edge_counts.data_ptr<int>() + num_voxels,
            ...
            if (num_voxels > 0)
            {
                ...
                cudaMemcpyAsync(&last_v_count, contour_counts.data_ptr<int>() + (num_voxels - 1), sizeof(int),
                                cudaMemcpyDeviceToHost, stream);
                cudaMemcpyAsync(&last_v_off, vert_offsets.data_ptr<int>() + (num_voxels - 1), sizeof(int),
                                cudaMemcpyDeviceToHost, stream);
                cudaMemcpyAsync(&last_e_count, edge_counts.data_ptr<int>() + (num_voxels - 1), sizeof(int),
                                cudaMemcpyDeviceToHost, stream);
                cudaMemcpyAsync(&last_e_off, edge_offsets.data_ptr<int>() + (num_voxels - 1), sizeof(int),
                                cudaMemcpyDeviceToHost, stream);
            ...
            const float3 *precomputed_ptr =
                (voxel_vertices.has_value() && voxel_vertices->defined() && voxel_vertices->numel() > 0)
            ...
            const float3 *dmc_ep_ptr = (edge_points.has_value() && edge_points->defined() && edge_points->numel() > 0)
                                           ? reinterpret_cast<const float3 *>(edge_points->data_ptr<float>())
                                           : nullptr;
            const float3 *dmc_en_ptr =
                (edge_normals.has_value() && edge_normals->defined() && edge_normals->numel() > 0)
                    ? reinterpret_cast<const float3 *>(edge_normals->data_ptr<float>())
                    : nullptr;
            ...
            dmc_extract_dual_vertices_and_edges_kernel<<<grid, block, 0, stream>>>(
                ...
                sdf.data_ptr<float>(), colors_ptr, precomputed_ptr, dmc_ep_ptr, dmc_en_ptr, num_channels,
                vert_offsets.data_ptr<int>(), edge_offsets.data_ptr<int>(), iso, project_iters, num_voxels,
                reinterpret_cast<float3 *>(out_vertices.data_ptr<float>()), out_colors_ptr,
            ...
            dmc_gather_quads_kernel<<<edge_grid, block, 0, stream>>>(
                ...
                reinterpret_cast<const uint32_t *>(dual_vert_and_edge.data_ptr<int>()), sdf.data_ptr<float>(),
                total_edge_instances, reinterpret_cast<int4 *>(out_quads.data_ptr<int>()),
            ...
            if (num_quads > 0)
            {
                dmc_quad_to_triangle_kernel<<<q_grid, block, 0, stream>>>(
                    ...
                    reinterpret_cast<const float3 *>(out_vertices.data_ptr<float>()), num_quads,
                    reinterpret_cast<int3 *>(out_triangles.data_ptr<int>()), tri_count_tensor.data_ptr<int>());
            }
        ...
        dual_marching_cubes_backward(const at::Tensor &grad_vertices, const c10::optional<at::Tensor> &grad_colors,
                                     const at::Tensor &grid_vertices, const at::Tensor &voxels, const at::Tensor &sdf,
                                     const c10::optional<at::Tensor> &colors, float iso, int project_iters)
        {
            at::cuda::CUDAGuard device_guard(grid_vertices.device());
            cudaStream_t stream = at::cuda::getCurrentCUDAStream();
            auto policy = thrust::cuda::par(at::cuda::ThrustAllocator()).on(stream);
            ...
            dmc_backward_kernel<<<grid, block, 0, stream>>>(
                reinterpret_cast<const float3 *>(grad_vertices.data_ptr<float>()), grad_colors_ptr,
                reinterpret_cast<const float3 *>(grid_vertices.data_ptr<float>()), voxels.data_ptr<int>(),
                sdf.data_ptr<float>(), colors_ptr, num_channels, vert_offsets.data_ptr<int>(), iso, num_voxels,
                grad_sdf.data_ptr<float>(), grad_colors_in_ptr);
