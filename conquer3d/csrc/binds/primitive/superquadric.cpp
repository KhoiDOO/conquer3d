...
compute_superquadric_mesh_wrapper(torch::Tensor scales, torch::Tensor exponents, torch::Tensor rotations,
                                  torch::Tensor translations, int64_t resolution, bool return_labels)
{
    CHECK_INPUT(scales);
    ...
    TORCH_CHECK(scales.dim() == 2 && scales.size(1) == 3, "scales must have shape (K, 3)");
    TORCH_CHECK(exponents.dim() == 2 && exponents.size(0) == num_quadrics && exponents.size(1) == 2,
                "exponents must have shape (K, 2)");
    TORCH_CHECK(rotations.dim() == 3 && rotations.size(0) == num_quadrics && rotations.size(1) == 3 &&
                    rotations.size(2) == 3,
    ...
    torch::Tensor stack_scratch = torch::empty({2 * num_quadrics * (n + 2) * frame_doubles}, options_f64);
    torch::Tensor azimuths = torch::empty({num_quadrics * (n + 1)}, options_f64);
    torch::Tensor polars = torch::empty({num_quadrics * n}, options_f64);

    sq::compute_superellipse_angles((uint32_t)num_quadrics, (uint32_t)n,
                                    ...
                                    azimuths.data_ptr<double>(), polars.data_ptr<double>());

    sq::compute_superquadric_vertices(
        (uint32_t)num_quadrics, (uint32_t)n, reinterpret_cast<const float3 *>(scales.data_ptr<float>()),
        reinterpret_cast<const float2 *>(exponents.data_ptr<float>()), rotations.data_ptr<float>(),
        reinterpret_cast<const float3 *>(translations.data_ptr<float>()), azimuths.data_ptr<double>(),
        polars.data_ptr<double>(), return_labels, reinterpret_cast<float3 *>(vertices.data_ptr<float>()),
        return_labels ? labels.data_ptr<int32_t>() : nullptr);

    sq::compute_superquadric_faces((uint32_t)num_quadrics, (uint32_t)n,
...
{
    m.def("compute_superquadric_mesh_func", &compute_superquadric_mesh_wrapper, py::arg("scales"), py::arg("exponents"),
          py::arg("rotations"), py::arg("translations"), py::arg("resolution") = 30, py::arg("return_labels") = false,
          R"pbdoc(
