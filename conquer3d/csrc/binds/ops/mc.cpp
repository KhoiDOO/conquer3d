...
/**
 * @brief Tensor-level entry point for the Marching Cubes extraction.
...
marching_cubes_wrapper(torch::Tensor grid_vertices, torch::Tensor voxels, torch::Tensor voxel_values,
                       std::optional<torch::Tensor> grid_normals, std::optional<torch::Tensor> grid_colors, float iso,
                       bool return_unique_edges)
{
    ...
    return mc::marching_cubes(num_voxels, p_grid_vertices, p_voxels, p_voxel_values, p_grid_normals, p_grid_colors, iso,
                              grid_vertices.options(), voxels.options(), return_unique_edges);
}

/**
 * @brief Tensor-level entry point for the Marching Cubes backward pass.
 ...
 */
void marching_cubes_backward_wrapper(torch::Tensor unique_edges, torch::Tensor grid_vertices,
                                     std::optional<torch::Tensor> grid_colors, torch::Tensor values,
                                     torch::Tensor adj_verts, std::optional<torch::Tensor> adj_colors,
                                     torch::Tensor adj_values, std::optional<torch::Tensor> adj_grid_colors, float iso)
{
    ...
    mc::backward(n_verts, p_unique_edges, p_grid_vertices, p_grid_colors, p_values, p_adj_verts, p_adj_colors,
                 p_adj_values, p_adj_grid_colors, iso);
}
...
{
    m.def("marching_cubes", &marching_cubes_wrapper, py::arg("grid_vertices"), py::arg("voxels"),
          py::arg("voxel_values"), py::arg("grid_normals") = std::nullopt, py::arg("grid_colors") = std::nullopt,
          py::arg("iso") = 0.0f, py::arg("return_unique_edges") = false,
          R"pbdoc(
          GPU-accelerated Marching Cubes algorithm for structured voxel grids (Lorensen & Cline 1987).

          Args:
              grid_vertices (torch.Tensor): (V, 3) float32 coordinates on CUDA.
              voxels (torch.Tensor): (N, 8) int32 corner indices per voxel cell.
              voxel_values (torch.Tensor): (V,) float32 scalar field values on CUDA.
          ...
          Example:
              >>> import torch
              >>> from conquer3d._C import marching_cubes
              >>> verts, tris, _, _, _ = marching_cubes(grid_vertices, voxels, voxel_values, iso=0.0)
          )pbdoc");
    m.def("marching_cubes_backward", &marching_cubes_backward_wrapper, py::arg("unique_edges"),
          py::arg("grid_vertices"), py::arg("grid_colors"), py::arg("values"), py::arg("adj_verts"),
          py::arg("adj_colors"), py::arg("adj_values"), py::arg("adj_grid_colors"), py::arg("iso"),
          R"pbdoc(
          Analytical backward gradient propagation for Marching Cubes on GPU.
          ...
          Example:
              >>> marching_cubes_backward(u_edges, g_verts, g_colors, vals, adj_verts, adj_colors, adj_vals, adj_g_colors, 0.0)
          )pbdoc");
}
