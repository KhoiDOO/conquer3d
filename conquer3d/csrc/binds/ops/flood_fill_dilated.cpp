...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, std::vector<int64_t>, std::vector<int64_t>> compute_flood_fill_dilated_cf_wrapper(
    ...
    auto res = ops::compute_flood_fill_dilated_cf(
        vertices, triangles, aabb_mins, aabb_maxs, bvh_children, object_ids,
        grid_min, grid_max, grid_res, dilation_radius, min_cavity_size, block_size, connectivity
    );
...
void bind_ops_flood_fill_dilated(py::module_& m) {
    m.def("compute_flood_fill_dilated_cf", &compute_flood_fill_dilated_cf_wrapper,
          ...
          py::arg("grid_res"), py::arg("dilation_radius") = 1, py::arg("min_cavity_size") = 64,
          py::arg("block_size") = std::vector<int64_t>{}, py::arg("connectivity") = 26,
          R"pbdoc(
          Computes a 2-level Hierarchical Boundary-Aware Dilated Flood Fill on GPU (< 15 MB VRAM at 1024^3).
          ...
          Args:
              ...
              dilation_radius (int, optional): Dilation radius k in voxels. Defaults to 1.
              min_cavity_size (int, optional): Minimum cavity voxel count. Defaults to 64.
              block_size (List[int], optional): Macro-block size [bx, by, bz]. If omitted, dynamically computed.
              connectivity (int, optional): Consensus neighborhood connectivity (6, 18, 26). Defaults to 26.
