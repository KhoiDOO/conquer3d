 ...
 */
std::tuple<torch::Tensor, torch::Tensor> one_sided_chamfer_distance_wrapper(torch::Tensor query_points,
                                                                            torch::Tensor reference_points)
{
    ...
    {
        return std::make_tuple(torch::empty({0}, query_points.options()),
    ...
    one_sided_chamfer_distance(num_query_points, p_query_points, num_reference_points, p_reference_points, p_distances,
                               p_indices);
...
one_sided_chamfer_distance_backward_wrapper(torch::Tensor query_points, torch::Tensor reference_points,
                                            torch::Tensor indices, torch::Tensor grad_distances, bool squared,
                                            bool compute_grad_query, bool compute_grad_reference)
{
    CHECK_INPUT(query_points);
    CHECK_INPUT(reference_points);
    CHECK_INPUT(indices);
    ...
    float3 *__restrict__ p_grad_query =
        (compute_grad_query && grad_query.defined()) ? (float3 *)grad_query.data_ptr<float>() : nullptr;
    float3 *__restrict__ p_grad_reference =
        (compute_grad_reference && grad_reference.defined()) ? (float3 *)grad_reference.data_ptr<float>() : nullptr;

    one_sided_chamfer_distance_backward(num_query_points, p_query_points, num_reference_points, p_reference_points,
                                        p_indices, p_grad_distances, squared, p_grad_query, p_grad_reference);
...
{
    m.def("one_sided_chamfer_distance", &one_sided_chamfer_distance_wrapper, py::arg("query_points"),
    ...
    m.def("one_sided_chamfer_distance_backward", &one_sided_chamfer_distance_backward_wrapper, py::arg("query_points"),
          py::arg("reference_points"), py::arg("indices"), py::arg("grad_distances"), py::arg("squared") = true,
          py::arg("compute_grad_query") = true, py::arg("compute_grad_reference") = true,
          R"pbdoc(
