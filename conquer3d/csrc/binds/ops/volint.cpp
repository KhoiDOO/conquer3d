 ...
 */
void single_view_volume_integral_bind(torch::Tensor grid_vertices, torch::Tensor sdf, torch::Tensor weight,
                                      std::optional<torch::Tensor> color_opt, torch::Tensor depth_image,
                                      std::optional<torch::Tensor> color_image_opt, torch::Tensor extrinsics,
                                      torch::Tensor intrinsics, float trunc_margin, int mode)
{
    ...
    float4x4 ex = make_float4x4(ex_data[0], ex_data[1], ex_data[2], ex_data[3], ex_data[4], ex_data[5], ex_data[6],
                                ex_data[7], ex_data[8], ex_data[9], ex_data[10], ex_data[11], ex_data[12], ex_data[13],
                                ex_data[14], ex_data[15]);

    const float *in_data = in_cpu.data_ptr<float>();
    float3x3 in = make_float3x3(in_data[0], in_data[1], in_data[2], in_data[3], in_data[4], in_data[5], in_data[6],
                                in_data[7], in_data[8]);

    single_view_volume_integral(num_vertices, grid_vertices_ptr, sdf_ptr, weight_ptr, color_ptr, depth_image_ptr,
                                color_image_ptr, image_width, image_height, ex, in, trunc_margin, mode);
}
...
{
    m.def("single_view_volume_integral", &single_view_volume_integral_bind, py::arg("grid_vertices"), py::arg("sdf"),
          py::arg("weight"), py::arg("color"), py::arg("depth_image"), py::arg("color_image"), py::arg("extrinsics"),
