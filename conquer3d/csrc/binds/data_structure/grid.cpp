...
create_voxel_grid(std::vector<float> grid_min, std::vector<float> grid_max, std::vector<int64_t> res,
                  std::string device_str, bool return_idx_grids)
{
    ...
    if (return_idx_grids)
    {
        ...
        opt_idx_grids = torch::stack({idx_grids_v[0].flatten(), idx_grids_v[1].flatten(), idx_grids_v[2].flatten()}, -1)
                            .contiguous();
    }
 ...
 * @return The constructed grid arrays as PyTorch tensors.
 */
torch::Tensor compute_grid_normal(torch::Tensor sdf, torch::Tensor grid_vertices, torch::Tensor idx_grids,
...
create_voxel_grid_from_tmesh(std::vector<float> grid_min, std::vector<float> grid_max, std::vector<int64_t> res,
                             TriangleMesh &tmesh, bool return_unique_vert_ids = true, int pad = 0,
                             bool return_normals = false, int normal_mode = 0, bool drop_empty_vertex_voxels = false)
{
    TORCH_CHECK(grid_min.size() == 3, "grid_min must have 3 elements.");
    TORCH_CHECK(grid_max.size() == 3, "grid_max must have 3 elements.");
    TORCH_CHECK(res.size() == 3, "res must have 3 elements.");
    ...
    if (active_voxel_ids.size(0) == 0)
    {
        return std::make_tuple(
            ...
            return_unique_vert_ids ? std::make_optional(torch::empty(
                                         {0}, torch::TensorOptions().dtype(torch::kInt64).device(vertices.device())))
                                   : std::nullopt,
            return_normals ? std::make_optional(torch::empty(
                                 {0, 3}, torch::TensorOptions().dtype(torch::kFloat32).device(vertices.device())))
                           : std::nullopt);
    }
    ...
    if (return_normals)
    {
        if (sparse_grid_vertices.size(0) > 0)
        {
            if (normal_mode == 0)
            {
                ...
                auto query_res = bvh.query_point(sparse_grid_vertices, vertices, triangles, false, false, 0);
            ...
            else if (normal_mode == 1)
            {
                ...
                auto query_res = bvh.query_point(sparse_grid_vertices, vertices, triangles, false, true, 0);
                auto closest_tri_ids = std::get<1>(query_res);
            ...
            {
                throw std::runtime_error("Unknown normal_mode. Supported modes are 0 (closest triangle face normal), 1 "
                                         "(interpolated vertex normal), 2 (displacement gradient vector).");
            }
        }
        else
        {
    ...
    return std::make_tuple(sparse_grid_vertices, remapped_voxels,
 ...
 */
std::tuple<torch::Tensor, torch::Tensor, std::optional<torch::Tensor>, std::optional<torch::Tensor>>
create_voxel_cloud_from_tmesh(std::vector<float> grid_min, std::vector<float> grid_max, std::vector<int64_t> res,
                              TriangleMesh &tmesh, bool return_unique_vert_ids = true, bool return_normals = false,
                              int normal_mode = 0)
{
    TORCH_CHECK(grid_min.size() == 3, "grid_min must have 3 elements.");
    TORCH_CHECK(grid_max.size() == 3, "grid_max must have 3 elements.");
    ...
    if (return_normals)
    {
        if (unique_grid_vertices.size(0) > 0)
        {
            ...
            if (normal_mode == 0)
            {
                auto query_res = bvh.query_point(unique_grid_vertices, vertices, triangles, false, false, 0);
                auto closest_tri_ids = std::get<1>(query_res);
                auto tri_normals = tmesh.get_triangle_normals();
                grid_normals = tri_normals.index_select(0, closest_tri_ids);
            }
            ...
            {
                auto query_res = bvh.query_point(unique_grid_vertices, vertices, triangles, false, true, 0);
                auto closest_tri_ids = std::get<1>(query_res);
                auto prj_pts = std::get<2>(query_res);
                auto vert_normals = tmesh.get_vertex_normals(1);
    ...
    auto vert_ids =
        torch::arange(vertices.size(0), torch::TensorOptions().device(vertices.device()).dtype(torch::kInt64));

    return std::make_tuple(unique_grid_vertices, remapped_voxels,
 ...
 */
torch::Tensor get_active_voxel_ids_from_depth_py(torch::Tensor depth_image, torch::Tensor c2w_tensor,
                                                 torch::Tensor intrinsics_inv_tensor, std::vector<float> grid_min,
                                                 std::vector<float> grid_max, std::vector<int64_t> res,
                                                 bool activate_neighbor = false, float trunc_margin = 0.0f)
{
    ...
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c)
            intrinsics_inv.m[r][c] = int_a[r][c];
    ...
    grid::get_active_voxel_ids_from_depth(
        num_pixels, depth_image.data_ptr<float>(), c2w, intrinsics_inv, image_width, image_height, g_min, g_max, g_res,
        out_voxel_ids.data_ptr<int64_t>(),
        reinterpret_cast<unsigned long long *>(valid_counter_tensor.data_ptr<int64_t>()), activate_neighbor,
        trunc_margin);
 ...
 */
std::tuple<torch::Tensor, torch::Tensor, std::optional<torch::Tensor>>
build_sparse_grid_from_active_voxels(torch::Tensor active_voxel_ids, std::vector<float> grid_min,
                                     std::vector<float> grid_max, std::vector<int64_t> res,
                                     bool return_unique_vert_ids = true)
{
    ...
    if (active_voxel_ids.size(0) == 0)
    {
        return std::make_tuple(
            ...
            return_unique_vert_ids
                ? std::make_optional(
                      torch::empty({0}, torch::TensorOptions().dtype(torch::kInt64).device(active_voxel_ids.device())))
                : std::nullopt);
    }
...
{
    m.def("create_voxel_grid", &create_voxel_grid, py::arg("grid_min"), py::arg("grid_max"), py::arg("res"),
    ...
    m.def("create_voxel_grid_from_tmesh", &create_voxel_grid_from_tmesh, py::arg("grid_min"), py::arg("grid_max"),
          py::arg("res"), py::arg("tmesh"), py::arg("return_unique_vert_ids") = true, py::arg("pad") = 0,
          py::arg("return_normals") = false, py::arg("normal_mode") = 0, py::arg("drop_empty_vertex_voxels") = false,
          R"pbdoc(
    ...
    m.def("create_voxel_cloud_from_tmesh", &create_voxel_cloud_from_tmesh, py::arg("grid_min"), py::arg("grid_max"),
          py::arg("res"), py::arg("tmesh"), py::arg("return_unique_vert_ids") = true, py::arg("return_normals") = false,
          py::arg("normal_mode") = 0,
          R"pbdoc(
    ...
    m.def("compute_grid_normal", &compute_grid_normal, py::arg("sdf"), py::arg("grid_vertices"), py::arg("idx_grids"),
          py::arg("res"),
          R"pbdoc(
    ...
    m.def("compute_active_voxels", &compute_active_voxels, py::arg("voxels"), py::arg("sdf"), py::arg("iso"),
          R"pbdoc(
    ...
    m.def("get_active_voxel_ids_from_depth", &get_active_voxel_ids_from_depth_py, py::arg("depth_image"),
          py::arg("c2w_tensor"), py::arg("intrinsics_inv_tensor"), py::arg("grid_min"), py::arg("grid_max"),
          py::arg("res"), py::arg("activate_neighbor") = false, py::arg("trunc_margin") = 0.0f,
          R"pbdoc(
    ...
    m.def("build_sparse_grid_from_active_voxels", &build_sparse_grid_from_active_voxels, py::arg("active_voxel_ids"),
          py::arg("grid_min"), py::arg("grid_max"), py::arg("res"), py::arg("return_unique_vert_ids") = true,
          R"pbdoc(
