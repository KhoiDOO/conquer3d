...
TriangleMesh::TriangleMesh(const torch::Tensor &in_vertices, const torch::Tensor &in_triangles,
...
void TriangleMesh::compute_triangle_normals()
{
    ...
    this->triangle_normals = torch::empty({static_cast<int64_t>(this->num_triangles), 3},
                                          torch::dtype(torch::kFloat32).device(this->vertices.device()));
    triangle_mesh::compute_triangle_normals(this->num_triangles,
...
void TriangleMesh::compute_vertex_normals(int mode)
{
    ...
    this->vertex_normals = torch::zeros({static_cast<int64_t>(num_vertices), 3},
                                        torch::dtype(torch::kFloat32).device(this->vertices.device()));

    triangle_mesh::compute_vertex_normals(
        num_vertices, this->num_triangles, reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
        reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
        reinterpret_cast<const float3 *>(this->get_triangle_normals().data_ptr<float>()),
        reinterpret_cast<float3 *>(this->vertex_normals.data_ptr<float>()), mode);
...
void TriangleMesh::compute_edge_normals()
{
    ...
    this->edge_normals = torch::empty({static_cast<int64_t>(this->num_triangles) * 3, 3},
                                      torch::dtype(torch::kFloat32).device(this->triangles.device()));
    triangle_mesh::compute_edge_normals(
        this->num_triangles, this->triangles,
        reinterpret_cast<const float3 *>(this->get_triangle_normals().data_ptr<float>()),
...
{
    if (!this->vertex_normals.defined() || !this->vertex_normals_mode.has_value() ||
        this->vertex_normals_mode.value() != mode)
    {
...
void TriangleMesh::compute_vertex_degrees()
{
    ...
    uint32_t num_vertices = this->vertices.size(0);
    this->vertex_degrees =
        torch::zeros({static_cast<int64_t>(num_vertices)}, torch::dtype(torch::kInt32).device(this->vertices.device()));

    uint32_t num_edges = this->edges.size(0);
    triangle_mesh::compute_vertex_degree(num_edges, reinterpret_cast<const int *>(this->edges.data_ptr<int>()),
...
void TriangleMesh::compute_vertex_lb_uniform()
{
    ...
    uint32_t num_vertices = this->vertices.size(0);
    this->vertex_lb_uniform = torch::zeros({static_cast<int64_t>(num_vertices), 3},
                                           torch::dtype(torch::kFloat32).device(this->vertices.device()));

    uint32_t num_edges = this->edges.size(0);
    triangle_mesh::compute_uniform_laplacian(num_vertices, num_edges,
...
void TriangleMesh::compute_voronoi_areas()
{
    ...
    uint32_t num_vertices = this->vertices.size(0);
    this->voronoi_areas = torch::zeros({static_cast<int64_t>(num_vertices)},
                                       torch::dtype(torch::kFloat32).device(this->vertices.device()));
    uint32_t num_triangles = this->triangles.size(0);

    triangle_mesh::compute_voronoi_areas(num_triangles, reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
                                         reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
...
void TriangleMesh::compute_gaussian_curvature()
{
    ...
    uint32_t num_vertices = this->vertices.size(0);
    this->gaussian_curvature = torch::zeros({static_cast<int64_t>(num_vertices)},
                                            torch::dtype(torch::kFloat32).device(this->vertices.device()));

    uint32_t num_triangles = this->triangles.size(0);
    auto vertex_angle_sum = torch::zeros({static_cast<int64_t>(num_vertices)},
                                         torch::dtype(torch::kFloat32).device(this->vertices.device()));

    triangle_mesh::compute_gaussian_curvature(num_vertices, num_triangles,
                                              reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
                                              reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
...
void TriangleMesh::compute_vertex_lb_cotangent()
{
    ...
    uint32_t num_vertices = this->vertices.size(0);
    this->vertex_lb_cotangent = torch::zeros({static_cast<int64_t>(num_vertices), 3},
                                             torch::dtype(torch::kFloat32).device(this->vertices.device()));

    uint32_t num_triangles = this->triangles.size(0);
    triangle_mesh::compute_cotangent_laplacian(num_vertices, num_triangles,
                                               reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
                                               reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                               reinterpret_cast<float *>(this->voronoi_areas.data_ptr<float>()),
...
{
    this->triangle_areas = torch::empty({static_cast<int64_t>(this->num_triangles)},
                                        torch::dtype(torch::kFloat32).device(this->vertices.device()));
    triangle_mesh::compute_triangle_areas(this->num_triangles,
...
MeshBVH TriangleMesh::build_bvh()
{
    if (!this->bvh.has_value())
    {
        ...
        triangle_mesh::compute_triangle_aabbs(this->num_triangles,
                                              reinterpret_cast<float3 *>(this->vertices.data_ptr<float>()),
                                              reinterpret_cast<int3 *>(this->triangles.data_ptr<int>()),
...
void TriangleMesh::build_flood_fill_data(std::optional<std::vector<float>> grid_min,
                                         std::optional<std::vector<float>> grid_max,
                                         std::optional<std::vector<int64_t>> res, int connectivity)
{
    this->build_bvh();
    ...
    this->flood_fill_mask = ops::compute_flood_fill(
        this->vertices, this->triangles, this->bvh.value().aabb_mins, this->bvh.value().aabb_maxs,
        this->bvh.value().bvh_children, this->bvh.value().object_ids, min_vals, max_vals, res_vals, connectivity);
...
void TriangleMesh::build_flood_fill_cf_data(std::optional<std::vector<float>> grid_min,
                                            std::optional<std::vector<float>> grid_max,
                                            std::optional<std::vector<int64_t>> res,
                                            std::optional<std::vector<int64_t>> block_size, int connectivity)
{
    this->build_bvh();
    std::vector<float> min_vals;
    std::vector<float> max_vals;
    ...
    auto cf_res =
        ops::compute_flood_fill_cf(this->vertices, this->triangles, this->bvh.value().aabb_mins,
                                   this->bvh.value().aabb_maxs, this->bvh.value().bvh_children,
                                   this->bvh.value().object_ids, min_vals, max_vals, res_vals, bs_vals, connectivity);
...
TriangleMesh::query_points(const torch::Tensor &query_pts, bool return_sdf, bool return_prj_pts, int sign_mode,
                           int distance_mode, bool return_occ)
{
    if (distance_mode == 0)
    {
        this->build_bvh();
        ...
        return this->bvh.value().query_point(
            ...
            (sign_mode == 2 || sign_mode == 4) ? std::optional<torch::Tensor>(this->get_vertex_normals(1))
                                               : std::nullopt,
...
TriangleMesh::get_ray_intersection(const torch::Tensor &ray_origins, const torch::Tensor &ray_dirs,
                                   bool return_distance)
{
    this->build_bvh();
    return this->bvh.value().get_ray_intersection(ray_origins, ray_dirs, this->vertices, this->triangles,
                                                  return_distance);
}

std::tuple<torch::Tensor, torch::Tensor, std::optional<torch::Tensor>, std::optional<torch::Tensor>>
TriangleMesh::sample_points(int num_points, bool uniform, bool return_normals, bool return_colors,
                            bool use_triangle_normal)
{
    ...
    if (!uniform)
    {
        tri_indices = torch::randint(0, this->num_triangles, {num_points},
                                     torch::TensorOptions().device(this->vertices.device()).dtype(torch::kInt64));
    }
    else
    {
    ...
    torch::Tensor r1_r2 =
        torch::rand({num_points, 2}, torch::TensorOptions().device(this->vertices.device()).dtype(torch::kFloat32));
    torch::Tensor out_points =
        torch::empty({num_points, 3}, torch::TensorOptions().device(this->vertices.device()).dtype(torch::kFloat32));
    ...
    if (return_normals)
    {
        out_normals = torch::empty({num_points, 3},
                                   torch::TensorOptions().device(this->vertices.device()).dtype(torch::kFloat32));
    ...
    if (return_colors)
    {
        ...
        out_colors = torch::empty({num_points, 3},
                                  torch::TensorOptions().device(this->vertices.device()).dtype(torch::kFloat32));
    ...
    triangle_mesh::sample_points_triangle_mesh(
        num_points, reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
        reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
        reinterpret_cast<const int64_t *>(tri_indices.data_ptr<int64_t>()),
        reinterpret_cast<const float2 *>(r1_r2.data_ptr<float>()), d_vertex_normals, d_triangle_normals,
        d_vertex_colors, reinterpret_cast<float3 *>(out_points.data_ptr<float>()), d_out_normals, d_out_colors);
...
std::tuple<float, float> TriangleMesh::get_quality()
{
    ...
    triangle_mesh::compute_quality(this->num_triangles,
                                   reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                   reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
...
torch::Tensor TriangleMesh::get_aspect_ratio(int mode)
{
    ...
    triangle_mesh::compute_aspect_ratio(this->num_triangles,
                                        reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                        reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()), mode,
...
torch::Tensor TriangleMesh::get_radii_ratio()
{
    ...
    triangle_mesh::compute_radii_ratio(this->num_triangles,
                                       reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                       reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
...
torch::Tensor TriangleMesh::get_triangle_regularity()
{
    ...
    triangle_mesh::compute_triangle_regularity(this->num_triangles,
                                               reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                               reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
...
torch::Tensor TriangleMesh::get_radius_edge_ratio()
{
    ...
    triangle_mesh::compute_radius_edge_ratio(this->num_triangles,
                                             reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                             reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
                                             reinterpret_cast<float *>(ratios.data_ptr<float>()));
...
torch::Tensor TriangleMesh::get_angle_deviation()
{
    ...
    triangle_mesh::compute_angle_deviation(this->num_triangles,
                                           reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                                           reinterpret_cast<const int3 *>(this->triangles.data_ptr<int>()),
...
void TriangleMesh::compute_edges_to_triangle_map()
{
    ...
    triangle_mesh::compute_edges_to_triangle_map(this->num_triangles, this->triangles, this->edges,
                                                 this->edge_to_triangle_offsets, this->edge_to_triangle_counts,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> TriangleMesh::get_edges_to_triangle_map()
{
    ...
    return std::make_tuple(this->edges, this->edge_to_triangle_offsets, this->edge_to_triangle_counts,
                           this->edge_to_triangle_indices);
}
...
void TriangleMesh::remove_triangles_by_mask(const torch::Tensor &keep_mask)
{
    ...
    TORCH_CHECK(keep_mask.dim() == 1 && keep_mask.size(0) == this->num_triangles,
                "keep_mask must have shape (num_triangles,)");
...
void TriangleMesh::fix_normals()
{
    ...
    triangle_mesh::fix_normals(this->num_triangles, reinterpret_cast<const float3 *>(this->vertices.data_ptr<float>()),
                               this->vertex_to_triangle_offsets, this->vertex_to_triangle_counts,
...
void TriangleMesh::compute_vertices_to_triangle_map()
{
    ...
    uint32_t num_vertices = this->vertices.size(0);
    triangle_mesh::build_vertices_to_triangle_map(num_vertices, this->num_triangles, this->triangles,
                                                  this->vertex_to_triangle_counts, this->vertex_to_triangle_offsets,
                                                  this->vertex_to_triangle_indices);
}
...
{
    this->compute_vertices_to_triangle_map();
    return triangle_mesh::get_non_manifold_vertices(this->vertices.size(0), this->triangles,
                                                    this->vertex_to_triangle_offsets, this->vertex_to_triangle_counts,
                                                    this->vertex_to_triangle_indices);
}
...
void bind_ds_triangle_mesh(py::module_ &m)
{
    py::class_<TriangleMesh>(m, "TriangleMesh", R"pbdoc(
        ...
        .def(py::init<const torch::Tensor &, const torch::Tensor &, std::optional<torch::Tensor>,
                      std::optional<torch::Tensor>>(),
             py::arg("in_vertices"), py::arg("in_triangles"), py::arg("in_vertex_normals") = std::nullopt,
        ...
        .def_property_readonly("num_triangles", &TriangleMesh::get_num_triangles,
                               "Total number of triangles in mesh (int).")
        .def_property_readonly("vertices", &TriangleMesh::get_vertices, "Mesh vertex coordinates (N, 3) float32.")
        .def_property_readonly(
            "vertex_normals", [](TriangleMesh &self) { return self.get_vertex_normals(0); },
        ...
        .def_property_readonly("vertex_degrees", &TriangleMesh::get_vertex_degrees,
                               "Vertex degrees (number of incident edges) (N,) int32.")
        .def_property_readonly("valence_567_percentage", &TriangleMesh::get_valence_567_percentage,
                               "Percentage of vertices with valence 5, 6, or 7.")
        .def_property_readonly("vertex_lb_uniform", &TriangleMesh::get_vertex_lb_uniform,
                               "Uniform Laplace-Beltrami operator (N, 3) float32.")
        ...
        .def_property_readonly("triangle_areas", &TriangleMesh::get_triangle_areas,
                               "Areas of each triangle (M,) float32.")
        .def_property_readonly("triangle_normals", &TriangleMesh::get_triangle_normals,
                               "Normals of each triangle (M, 3) float32.")
        .def_property_readonly("edge_normals", &TriangleMesh::get_edge_normals,
                               "Directed edge normals (3*M, 3) float32 for sign queries.")
        .def_property_readonly("edge_normal", &TriangleMesh::get_edge_normal, "Alias for edge_normals.")
        .def("get_edge_normals", &TriangleMesh::get_edge_normals,
             "Get edge normals (3*M, 3) float32 for pseudonormal sign queries.")
        .def("get_edge_normal", &TriangleMesh::get_edge_normal, "Alias for get_edge_normals.")
        .def("compute_edge_normals", &TriangleMesh::compute_edge_normals,
        ...
        .def_property_readonly("bvh", &TriangleMesh::build_bvh,
                               "The Bounding Volume Hierarchy (MeshBVH) built for this mesh.")
        .def("build_bvh", &TriangleMesh::build_bvh, "Builds and returns the MeshBVH for the mesh.")
        .def("build_flood_fill_data", &TriangleMesh::build_flood_fill_data, py::arg("grid_min") = py::none(),
             py::arg("grid_max") = py::none(), py::arg("res") = py::none(), py::arg("connectivity") = 6,
             R"pbdoc(
        ...
        .def_property_readonly("flood_fill_mask", &TriangleMesh::get_flood_fill_mask,
                               "Pre-computed flood fill int32 mask tensor.")
        .def_property_readonly("flood_grid_min", &TriangleMesh::get_flood_grid_min,
                               "Flood grid bounding box min coordinates.")
        .def_property_readonly("flood_grid_max", &TriangleMesh::get_flood_grid_max,
                               "Flood grid bounding box max coordinates.")
        .def_property_readonly("flood_grid_res", &TriangleMesh::get_flood_grid_res, "Flood grid vertex resolution.")
        .def("build_flood_fill_cf_data", &TriangleMesh::build_flood_fill_cf_data, py::arg("grid_min") = py::none(),
             py::arg("grid_max") = py::none(), py::arg("res") = py::none(), py::arg("block_size") = py::none(),
        ...
        .def_property_readonly("cf_coarse_mask", &TriangleMesh::get_cf_coarse_mask,
        ...
        .def(
            ...
            [](TriangleMesh &self, const torch::Tensor &ray_origins, const torch::Tensor &ray_dirs,
        ...
        .def(
            "query_points",
            [](TriangleMesh &self, const torch::Tensor &query_pts, bool return_sdf, bool return_prj_pts, int sign_mode,
               int distance_mode, bool return_occ) -> py::object
            {
                auto result =
                    self.query_points(query_pts, return_sdf, return_prj_pts, sign_mode, distance_mode, return_occ);
                if (return_occ)
                {
                    return py::cast(result);
                }
        ...
        .def("compute_vertices_to_triangle_map", &TriangleMesh::compute_vertices_to_triangle_map,
             "Computes vertex-to-triangle connectivity map.")
        ...
        .def_property_readonly("num_isolated_vertices", &TriangleMesh::get_num_isolated_vertices,
                               "Total number of isolated vertices (int).")
        ...
        .def_property_readonly("euler_characteristic", &TriangleMesh::get_euler_characteristic,
                               "The Euler characteristic (V - E + F) of the mesh (int).")
