...
namespace grid
{
     ...
     */
    __global__ void get_active_voxel_ids_from_depth_kernel(const int num_pixels, const float *depth_image,
                                                           const float4x4 c2w, const float3x3 intrinsics_inv,
        ...
        for (int i = i_min; i <= i_max; ++i)
        {
            for (int j = j_min; j <= j_max; ++j)
            {
                for (int k = k_min; k <= k_max; ++k)
                {
                    ...
                    {
                        int64_t voxel_id =
                            (int64_t)i * (res.y - 1) * (res.z - 1) + (int64_t)j * (res.z - 1) + (int64_t)k;
                                         ...
                                         bool activate_neighbor, float trunc_margin)
    {
        ...
        get_active_voxel_ids_from_depth_kernel<<<grid_size, block_size>>>(
            num_pixels, depth_image, c2w, intrinsics_inv, image_width, image_height, grid_min, grid_max, res,
            out_voxel_ids, valid_counter, activate_neighbor, trunc_margin);
    }
     ...
     */
    __global__ void quantize_vertices_to_voxel_ids_kernel(const float3 *__restrict__ vertices, int num_vertices,
                                                          float3 grid_min, float3 grid_spacing, int3 num_cells,
                                                    ...
                                                    std::vector<float> grid_max, std::vector<int64_t> res)
    {
        ...
        int3 num_cells = make_int3(static_cast<int>(rx - 1), static_cast<int>(ry - 1), static_cast<int>(rz - 1));
        ...
        quantize_vertices_to_voxel_ids_kernel<<<blocks, threads>>>((const float3 *)vertices.data_ptr<float>(),
                                                                   num_vertices, f_min, f_spacing, num_cells,
     ...
     */
    __global__ void create_voxel_cloud_corners_kernel(const float3 *__restrict__ vertices, int num_vertices,
                                                      float3 voxel_spacing, float3 *__restrict__ out_corners)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_vertices)
            return;
                                                                        ...
                                                                        std::vector<int64_t> res)
    {
        ...
        if (num_vertices > 0)
        {
            ...
            create_voxel_cloud_corners_kernel<<<blocks, threads>>>((const float3 *)vertices.data_ptr<float>(),
                                                                   num_vertices, f_spacing,
