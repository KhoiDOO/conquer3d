...
namespace mesh_bvh
{
     ...
     */
    __global__ void filter_self_intersections_kernel(const int num_pairs, const int64_t *query_ids,
                                                     const int64_t *object_ids, const float3 *vertices,
                                                     const int3 *triangles, int64_t *out_query_ids,
                                                     int64_t *out_object_ids, int64_t *valid_counter)
    {
        ...
        if (triA.x == triB.x || triA.x == triB.y || triA.x == triB.z || triA.y == triB.x || triA.y == triB.y ||
            triA.y == triB.z || triA.z == triB.x || triA.z == triB.y || triA.z == triB.z)
        {
            return;
        }
     ...
     */
    void filter_self_intersections(const int num_pairs, const int64_t *query_ids, const int64_t *object_ids,
                                   const float3 *vertices, const int3 *triangles, int64_t *out_query_ids,
                                   int64_t *out_object_ids, int64_t *valid_counter)
    {
        int threads = NTHREADS;
        int blocks = (num_pairs + threads - 1) / threads;

        filter_self_intersections_kernel<<<blocks, threads>>>(num_pairs, query_ids, object_ids, vertices, triangles,
                                                              out_query_ids, out_object_ids, valid_counter);
    }
     ...
     */
    __global__ void filter_ray_triangle_intersections_kernel(const int num_pairs, const int64_t *query_ids,
                                                             const int64_t *object_ids, const float3 *ray_origins,
                                                             const float3 *ray_dirs, const float3 *vertices,
                                                             const int3 *triangles, int64_t *out_query_ids,
                                                             int64_t *out_object_ids, float3 *out_intersect_pts,
                                                             float *out_distances, bool return_distance,
     ...
     */
    void filter_ray_triangle_intersections(const int num_pairs, const int64_t *query_ids, const int64_t *object_ids,
                                           const float3 *ray_origins, const float3 *ray_dirs, const float3 *vertices,
                                           const int3 *triangles, int64_t *out_query_ids, int64_t *out_object_ids,
                                           float3 *out_intersect_pts, float *out_distances, bool return_distance,
                                           int64_t *valid_counter)
    {
        int threads = NTHREADS;
        int blocks = (num_pairs + threads - 1) / threads;
        ...
        filter_ray_triangle_intersections_kernel<<<blocks, threads>>>(
            num_pairs, query_ids, object_ids, ray_origins, ray_dirs, vertices, triangles, out_query_ids, out_object_ids,
            out_intersect_pts, return_distance ? out_distances : nullptr, return_distance, valid_counter);
    }
     ...
     */
    __device__ __forceinline__ float compute_sign_ray_parity(const float3 &p, const int best_tri_id,
                                                             const float3 *bvh_aabb_mins, const float3 *bvh_aabb_maxs,
                                                             const int2 *bvh_children, const int *object_ids,
                                                             ...
                                                             const uint32_t num_objects)
    {
        ...
        float3 ray_dir = maths::normalize_safe(centroid - p, make_float3(1.0f, 0.0f, 0.0f), 1e-6f);
        ...
        bvh::traverse(
            ...
            {
                float t_hit_aabb;
                return ray2.is_intersect_aabb(bvh_aabb_mins[node_idx], bvh_aabb_maxs[node_idx], t_hit_aabb);
            },
            [&](int leaf_idx)
            {
    ...
    __device__ __forceinline__ float compute_solid_angle_tri(float3 p, float3 a, float3 b, float3 c)
    {
        ...
        float den = a_len * b_len * c_len + maths::dot(a_v, b_v) * c_len + maths::dot(b_v, c_v) * a_len +
                    maths::dot(c_v, a_v) * b_len;
     ...
     */
    __device__ __forceinline__ float compute_fast_winding_number(const float3 &p, const float3 *bvh_aabb_mins,
                                                                 const float3 *bvh_aabb_maxs, const int2 *bvh_children,
                                                                 const int *object_ids, const float3 *vertices,
                                                                 const int3 *triangles, const WindingData *winding_data,
                                                                 const uint32_t num_objects, const float accuracy_scale)
    {
    ...
    compute_pseudonormal_sign(const float3 &p, const float3 &best_pt, const int best_tri_id, const float3 *vertices,
                              const int3 *triangles, const float3 *pseudonormal_vertices,
                              const float3 *pseudonormal_edges, const float3 *pseudonormal_faces)
    {
        int3 tri = triangles[best_tri_id];
        float3 v0 = vertices[tri.x];
        float3 v1 = vertices[tri.y];
    ...
    __global__ void query_point_mesh_bvh_kernel(
        const int num_queries, const int num_objects, const float3 *__restrict__ query_points,
        const float3 *__restrict__ vertices, const int3 *__restrict__ triangles,
        const float3 *__restrict__ bvh_aabb_mins, const float3 *__restrict__ bvh_aabb_maxs,
        const int2 *__restrict__ bvh_children, const int *__restrict__ object_ids,
        const WindingData *__restrict__ winding_data, const float3 *__restrict__ pseudonormal_vertices,
        const float3 *__restrict__ pseudonormal_edges, const float3 *__restrict__ pseudonormal_faces,
        int64_t *__restrict__ out_query_ids, int64_t *__restrict__ out_object_ids,
        float3 *__restrict__ out_projected_pts, float *__restrict__ out_distances, bool return_sdf, bool return_prj_pts,
        int sign_mode, const int8_t *__restrict__ flood_mask, float3 flood_min, float3 flood_spacing, int3 flood_dims,
        const int8_t *__restrict__ cf_coarse_mask, const int32_t *__restrict__ cf_boundary_lookup,
        const int8_t *__restrict__ cf_fine_masks, int3 cf_block_size, int3 cf_coarse_dims)
    {
        ...
        while (stack_ptr >= 0)
        {
            ...
            else
            {
                if (stack_ptr + 2 < BVH_STACK_SIZE)
                {
                    ...
                    float dist_l =
                        aabb::compute_squared_distance(p, bvh_aabb_mins[children.x], bvh_aabb_maxs[children.x]);
                    float dist_r =
                        aabb::compute_squared_distance(p, bvh_aabb_mins[children.y], bvh_aabb_maxs[children.y]);
        ...
        if (return_sdf && best_tri_id >= 0)
        {
            if (sign_mode == 0)
            {
                dist *= compute_sign_ray_parity(p, best_tri_id, bvh_aabb_mins, bvh_aabb_maxs, bvh_children, object_ids,
                                                vertices, triangles, num_objects);
            }
            else if (sign_mode == 1)
            {
                float wn = compute_fast_winding_number(p, bvh_aabb_mins, bvh_aabb_maxs, bvh_children, object_ids,
                                                       vertices, triangles, winding_data, num_objects, 2.0f);
                dist *= (wn >= 0.5f) ? -1.0f : 1.0f;
            }
            else if (sign_mode == 2)
            {
                dist *= compute_pseudonormal_sign(p, best_pt, best_tri_id, vertices, triangles, pseudonormal_vertices,
                                                  pseudonormal_edges, pseudonormal_faces);
            }
            else if (sign_mode == 3)
            {
                ...
                if (i >= 0 && i < flood_dims.x && j >= 0 && j < flood_dims.y && k >= 0 && k < flood_dims.z &&
                    flood_mask != nullptr)
                {
            ...
            else if (sign_mode == 4)
            {
                ...
                else
                {
                    ...
                    dist *= compute_pseudonormal_sign(p, best_pt, best_tri_id, vertices, triangles,
     ...
     */
    void query_point_mesh_bvh(const int num_queries, const int num_objects, const float3 *query_points,
                              const float3 *vertices, const int3 *triangles, const float3 *bvh_aabb_mins,
                              const float3 *bvh_aabb_maxs, const int2 *bvh_children, const int *object_ids,
                              const WindingData *winding_data, const float3 *pseudonormal_vertices,
                              const float3 *pseudonormal_edges, const float3 *pseudonormal_faces,
                              int64_t *out_query_ids, int64_t *out_object_ids, float3 *out_projected_pts,
                              float *out_distances, bool return_sdf, bool return_prj_pts, int sign_mode,
                              const int8_t *flood_mask, float3 flood_min, float3 flood_spacing, int3 flood_dims,
                              const int8_t *cf_coarse_mask, const int32_t *cf_boundary_lookup,
                              const int8_t *cf_fine_masks, int3 cf_block_size, int3 cf_coarse_dims)
    {
        int threads = NTHREADS;
        int blocks = (num_queries + threads - 1) / threads;

        query_point_mesh_bvh_kernel<<<blocks, threads>>>(
            num_queries, num_objects, query_points, vertices, triangles, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
            object_ids, winding_data, pseudonormal_vertices, pseudonormal_edges, pseudonormal_faces, out_query_ids,
            out_object_ids, out_projected_pts, out_distances, return_sdf, return_prj_pts, sign_mode, flood_mask,
            flood_min, flood_spacing, flood_dims, cf_coarse_mask, cf_boundary_lookup, cf_fine_masks, cf_block_size,
            cf_coarse_dims);
    }
     ...
     */
    __global__ void
    bottom_up_winding_data_kernel(const int num_objects, const int *__restrict__ object_ids,
                                  const float3 *__restrict__ vertices, const int3 *__restrict__ triangles,
                                  const int *__restrict__ bvh_parents, const int2 *__restrict__ bvh_children,
                                  WindingData *__restrict__ winding_data, int *__restrict__ atomic_flags)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_objects)
            return;
     ...
     */
    void bottom_up_winding_data(const int num_objects, const int *object_ids, const float3 *vertices,
                                const int3 *triangles, const int *bvh_parents, const int2 *bvh_children,
                                WindingData *winding_data)
    {
        ...
        bottom_up_winding_data_kernel<<<blocks, threads>>>(num_objects, object_ids, vertices, triangles, bvh_parents,
                                                           bvh_children, winding_data,
     ...
     */
    __global__ void
    query_voxel_mesh_bvh_kernel(const int num_queries, const int num_objects, const float3 *__restrict__ query_mins,
                                const float3 *__restrict__ query_maxs, const float3 *__restrict__ vertices,
                                const int3 *__restrict__ triangles, const float3 *__restrict__ bvh_aabb_mins,
                                const float3 *__restrict__ bvh_aabb_maxs, const int2 *__restrict__ bvh_children,
                                const int *__restrict__ object_ids, bool *__restrict__ out_intersect)
    {
        int q_idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (q_idx >= num_queries)
            return;
        ...
        bvh::traverse(
            num_objects, bvh_children, [&](int node_idx)
            { return aabb::test_aabb_overlap(q_min, q_max, bvh_aabb_mins[node_idx], bvh_aabb_maxs[node_idx]); },
            [&](int leaf_idx)
            {
                int3 tri = triangles[object_ids[leaf_idx]];
                Triangle T(vertices[tri.x], vertices[tri.y], vertices[tri.z]);
     ...
     */
    void query_voxel_mesh_bvh(const int num_queries, const int num_objects, const float3 *query_mins,
                              const float3 *query_maxs, const float3 *vertices, const int3 *triangles,
                              const float3 *bvh_aabb_mins, const float3 *bvh_aabb_maxs, const int2 *bvh_children,
                              const int *object_ids, bool *out_intersect)
    {
        int threads = NTHREADS;
        int blocks = (num_queries + threads - 1) / threads;

        query_voxel_mesh_bvh_kernel<<<blocks, threads>>>(num_queries, num_objects, query_mins, query_maxs, vertices,
                                                         triangles, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
                                                         object_ids, out_intersect);
    }
     ...
     */
    __global__ void count_active_voxels_mesh_bvh_kernel(const int3 res, const float3 grid_min, const float3 voxel_size,
                                                        const int num_objects, const float3 *__restrict__ vertices,
                                                        ...
                                                        unsigned long long *__restrict__ active_counter)
    {
        ...
        float3 q_min =
            make_float3(grid_min.x + vi * voxel_size.x, grid_min.y + vj * voxel_size.y, grid_min.z + vk * voxel_size.z);

        float3 q_max = make_float3(q_min.x + voxel_size.x, q_min.y + voxel_size.y, q_min.z + voxel_size.z);
    ...
    __global__ void collect_active_voxels_mesh_bvh_kernel(
        const int3 res, const float3 grid_min, const float3 voxel_size, const int num_objects,
        const float3 *__restrict__ vertices, const int3 *__restrict__ triangles,
        const float3 *__restrict__ bvh_aabb_mins, const float3 *__restrict__ bvh_aabb_maxs,
        const int2 *__restrict__ bvh_children, const int *__restrict__ object_ids,
        unsigned long long *__restrict__ active_counter, int64_t *__restrict__ out_active_ids)
    {
        int64_t rx = res.x;
        int64_t ry = res.y;
        int64_t rz = res.z;
     ...
     */
    void count_active_voxels_mesh_bvh(const int3 res, const float3 grid_min, const float3 voxel_size,
                                      const int num_objects, const float3 *vertices, const int3 *triangles,
                                      const float3 *bvh_aabb_mins, const float3 *bvh_aabb_maxs,
                                      const int2 *bvh_children, const int *object_ids, int64_t *active_counter)
    {
        int64_t num_queries = (int64_t)res.x * res.y * res.z;
        int threads = NTHREADS;
        int blocks = (num_queries + threads - 1) / threads;

        count_active_voxels_mesh_bvh_kernel<<<blocks, threads>>>(res, grid_min, voxel_size, num_objects, vertices,
                                                                 triangles, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
                                                                 object_ids, (unsigned long long *)active_counter);
    }
     ...
     */
    void collect_active_voxels_mesh_bvh(const int3 res, const float3 grid_min, const float3 voxel_size,
                                        const int num_objects, const float3 *vertices, const int3 *triangles,
                                        const float3 *bvh_aabb_mins, const float3 *bvh_aabb_maxs,
                                        const int2 *bvh_children, const int *object_ids, int64_t *active_counter,
                                        int64_t *out_active_ids)
    {
        ...
        collect_active_voxels_mesh_bvh_kernel<<<blocks, threads>>>(
            res, grid_min, voxel_size, num_objects, vertices, triangles, bvh_aabb_mins, bvh_aabb_maxs, bvh_children,
            object_ids, (unsigned long long *)active_counter, out_active_ids);
    }
} // namespace mesh_bvh
