...
namespace triangle_mesh
{
     ...
     */
    __global__ void compute_triangle_normals_kernel(const uint32_t num_triangles, const float3 *__restrict__ vertices,
     ...
     */
    __global__ void compute_triangle_areas_kernel(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                                  const int3 *__restrict__ triangles,
    ...
    struct MetricQuality
    {
        ...
        __device__ __forceinline__ float operator()(const Triangle &T) const { return T.compute_quality(); }
    };
    ...
    struct MetricRegularity
    {
        ...
        __device__ __forceinline__ float operator()(const Triangle &T) const { return T.compute_triangle_regularity(); }
    };
    ...
    struct MetricRadiusEdgeRatio
    {
        ...
        __device__ __forceinline__ float operator()(const Triangle &T) const { return T.compute_radius_edge_ratio(); }
    };
    ...
    struct MetricAngleDeviation
    {
        ...
        __device__ __forceinline__ float operator()(const Triangle &T) const { return T.compute_angle_deviation(); }
    };
    ...
    struct MetricRadiiRatio
    {
        ...
        __device__ __forceinline__ float operator()(const Triangle &T) const { return T.compute_radii_ratio(); }
    };
    ...
    __global__ void per_triangle_metric_kernel(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                               const int3 *__restrict__ triangles, float *__restrict__ out,
                                               Metric metric)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_triangles)
     ...
     */
    template <class Metric>
    __host__ void launch_per_triangle_metric(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                             const int3 *__restrict__ triangles, float *__restrict__ out, Metric metric)
    {
        if (num_triangles == 0)
            return;
        ...
        per_triangle_metric_kernel<Metric><<<blocks, threads>>>(num_triangles, vertices, triangles, out, metric);
    }
     ...
     * attains the minimum.
     */
    __global__ void compute_aspect_ratio_kernel(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                                const int3 *__restrict__ triangles, int mode,
     ...
     */
    __global__ void compute_triangle_aabbs_kernel(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                                  const int3 *__restrict__ triangles, float3 *__restrict__ aabb_mins,
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_triangle_normals(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                           const int3 *__restrict__ triangles, float3 *__restrict__ triangle_normals)
    {
        if (num_triangles == 0)
            return;
        ...
        compute_triangle_normals_kernel<<<blocks, threads>>>(num_triangles, vertices, triangles, triangle_normals);
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_triangle_areas(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                         const int3 *__restrict__ triangles, float *__restrict__ triangle_areas)
    {
        if (num_triangles == 0)
            return;
        ...
        compute_triangle_areas_kernel<<<blocks, threads>>>(num_triangles, vertices, triangles, triangle_areas);
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_quality(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                  const int3 *__restrict__ triangles, float *__restrict__ qualities)
    {
        launch_per_triangle_metric(num_triangles, vertices, triangles, qualities, MetricQuality{});
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_aspect_ratio(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                       const int3 *__restrict__ triangles, int mode, float *__restrict__ aspect_ratios)
    {
        if (num_triangles == 0)
            return;
        ...
        compute_aspect_ratio_kernel<<<blocks, threads>>>(num_triangles, vertices, triangles, mode, aspect_ratios);
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_radii_ratio(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                      const int3 *__restrict__ triangles, float *__restrict__ ratios)
    {
        launch_per_triangle_metric(num_triangles, vertices, triangles, ratios, MetricRadiiRatio{});
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_triangle_regularity(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                              const int3 *__restrict__ triangles, float *__restrict__ regularities)
    {
        launch_per_triangle_metric(num_triangles, vertices, triangles, regularities, MetricRegularity{});
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_radius_edge_ratio(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                            const int3 *__restrict__ triangles, float *__restrict__ ratios)
    {
        launch_per_triangle_metric(num_triangles, vertices, triangles, ratios, MetricRadiusEdgeRatio{});
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_angle_deviation(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                          const int3 *__restrict__ triangles, float *__restrict__ deviations)
    {
        launch_per_triangle_metric(num_triangles, vertices, triangles, deviations, MetricAngleDeviation{});
    }
     ...
     * @param[in] triangles Device array of triangle vertex indices.
     */
    __host__ void compute_triangle_aabbs(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                         const int3 *__restrict__ triangles, float3 *__restrict__ aabb_mins,
                                         float3 *__restrict__ aabb_maxs)
    {
        if (num_triangles == 0)
        ...
        compute_triangle_aabbs_kernel<<<blocks, threads>>>(num_triangles, vertices, triangles, aabb_mins, aabb_maxs);
    }
     ...
     */
    __global__ void compute_vertex_normals_kernel(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                                                  const int3 *__restrict__ triangles,
                                                  const float3 *__restrict__ triangle_normals,
                                                  float3 *__restrict__ vertex_normals, int mode)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_triangles)
        {
     ...
     * and yields NaNs.
     */
    __global__ void normalize_vertex_normals_kernel(const uint32_t num_vertices, float3 *__restrict__ vertex_normals)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_vertices)
        {
     ...
     */
    __host__ void compute_vertex_normals(const uint32_t num_vertices, const uint32_t num_triangles,
                                         const float3 *__restrict__ vertices, const int3 *__restrict__ triangles,
                                         const float3 *__restrict__ triangle_normals,
                                         float3 *__restrict__ vertex_normals, int mode)
    {
        ...
        compute_vertex_normals_kernel<<<blocks, threads>>>(num_triangles, vertices, triangles, triangle_normals,
                                                           vertex_normals, mode);

        int blocks_vert = (num_vertices + threads - 1) / threads;
        normalize_vertex_normals_kernel<<<blocks_vert, threads>>>(num_vertices, vertex_normals);
    }
     ...
     */
    __global__ void extract_edge_slots_kernel(const uint32_t num_triangles, const int3 *__restrict__ triangles,
                                              Edge *__restrict__ edge_keys, int *__restrict__ edge_indices)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_triangles)
        {
     ...
     */
    __global__ void compute_edge_normals_kernel(const uint32_t num_edges, const Edge *__restrict__ sorted_edge_keys,
     ...
     * watertight mesh.
     */
    __host__ void compute_edge_normals(const uint32_t num_triangles, const torch::Tensor &triangles,
                                       const float3 *__restrict__ triangle_normals, float3 *__restrict__ edge_normals)
    {
        if (num_triangles == 0)
            return;
        ...
        extract_edge_slots_kernel<<<blocks_tri, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            ...
            reinterpret_cast<Edge *>(edge_keys.data_ptr<int64_t>()), edge_indices.data_ptr<int>());

        thrust::sort_by_key(policy, reinterpret_cast<Edge *>(edge_keys.data_ptr<int64_t>()),
        ...
        compute_edge_normals_kernel<<<blocks_edge, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            num_edges, reinterpret_cast<const Edge *>(edge_keys.data_ptr<int64_t>()), edge_indices.data_ptr<int>(),
            triangle_normals, edge_normals);
    }
     ...
     */
    __global__ void extract_edges_kernel(const uint32_t num_triangles, const int3 *triangles, Edge *edge_keys,
     ...
     */
    __global__ void unpack_edges_kernel(const uint32_t num_unique_edges, const Edge *unique_edge_keys,
     ...
     */
    __host__ void compute_edges_to_triangle_map(const uint32_t num_triangles, const torch::Tensor &triangles,
                                                torch::Tensor &out_unique_edges, torch::Tensor &out_offsets,
                                                torch::Tensor &out_counts, torch::Tensor &out_sorted_triangle_indices)
    {
        if (num_triangles == 0)
            return;
        ...
        extract_edges_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            num_triangles, (const int3 *)triangles.data_ptr<int>(), (Edge *)edge_keys.data_ptr<int64_t>(),
            out_sorted_triangle_indices.data_ptr<int>());

        thrust::sort_by_key(policy, (Edge *)edge_keys.data_ptr<int64_t>(),
        ...
        auto new_end = thrust::reduce_by_key(policy, (Edge *)edge_keys.data_ptr<int64_t>(),
                                             (Edge *)edge_keys.data_ptr<int64_t>() + num_edges, ones.data_ptr<int>(),
                                             (Edge *)unique_keys.data_ptr<int64_t>(), out_counts.data_ptr<int>());
        ...
        if (blocks2 > 0)
        {
            unpack_edges_kernel<<<blocks2, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                num_unique_edges, (Edge *)unique_keys.data_ptr<int64_t>(), out_unique_edges.data_ptr<int>());
        }
    }
     ...
     */
    __global__ void compute_vertex_triangle_counts_kernel(const uint32_t num_triangles, const int3 *triangles,
                                                          int *counts)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_triangles)
     ...
     */
    __global__ void compute_vertex_triangle_indices_kernel(const uint32_t num_triangles, const int3 *triangles,
                                                           int *current_offsets, int *indices)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_triangles)
        {
     ...
     */
    void build_vertices_to_triangle_map(const uint32_t num_vertices, const uint32_t num_triangles,
                                        const torch::Tensor &triangles, torch::Tensor &out_counts,
                                        torch::Tensor &out_offsets, torch::Tensor &out_indices)
    {
        if (num_triangles == 0 || num_vertices == 0)
            return;
        ...
        compute_vertex_triangle_counts_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            num_triangles, (const int3 *)triangles.data_ptr<int>(), out_counts.data_ptr<int>());
        ...
        compute_vertex_triangle_indices_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            num_triangles, (const int3 *)triangles.data_ptr<int>(), current_offsets.data_ptr<int>(),
     ...
     */
    __global__ void get_non_manifold_vertices_kernel(const uint32_t num_vertices, const int3 *triangles,
                                                     const int *v2t_offsets, const int *v2t_counts,
                                                     const int *v2t_indices, bool *out_is_non_manifold)
    {
        ...
        while (frontier != 0)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                if ((visited & (1ULL << i)) == 0)
                {
                    ...
                    if (n1_current == n1_other || n1_current == n2_other || n2_current == n1_other ||
                        n2_current == n2_other)
                    {
                        visited |= (1ULL << i);
                        frontier |= (1ULL << i);
     ...
     */
    torch::Tensor get_non_manifold_vertices(const uint32_t num_vertices, const torch::Tensor &triangles,
                                            const torch::Tensor &v2t_offsets, const torch::Tensor &v2t_counts,
                                            const torch::Tensor &v2t_indices)
    {
        ...
        if (num_vertices == 0)
            return torch::empty({0}, torch::TensorOptions().dtype(torch::kInt64).device(triangles.device()));

        int threads = NTHREADS;
        int blocks = (num_vertices + threads - 1) / threads;

        get_non_manifold_vertices_kernel<<<blocks, threads>>>(
            num_vertices, (const int3 *)triangles.data_ptr<int>(), v2t_offsets.data_ptr<int>(),
            v2t_counts.data_ptr<int>(), v2t_indices.data_ptr<int>(), out_is_non_manifold.data_ptr<bool>());
     ...
     */
    __global__ void
    sample_points_triangle_mesh_kernel(const int num_points, const float3 *__restrict__ vertices,
                                       const int3 *__restrict__ triangles, const int64_t *__restrict__ tri_indices,
                                       const float2 *__restrict__ r1_r2, const float3 *__restrict__ vertex_normals,
                                       const float3 *__restrict__ triangle_normals,
                                       const float3 *__restrict__ vertex_colors, float3 *__restrict__ out_points,
                                       float3 *__restrict__ out_normals, float3 *__restrict__ out_colors)
    {
        ...
        if (out_normals)
        {
            ...
            else if (vertex_normals)
            {
                ...
                float3 n = make_float3(n0.x * u + n1.x * v + n2.x * w, n0.y * u + n1.y * v + n2.y * w,
        ...
        if (out_colors && vertex_colors)
        {
            ...
            out_colors[idx] = make_float3(c0.x * u + c1.x * v + c2.x * w, c0.y * u + c1.y * v + c2.y * w,
     ...
     * faces.
     */
    __host__ void sample_points_triangle_mesh(const int num_points, const float3 *__restrict__ vertices,
                                              const int3 *__restrict__ triangles,
                                              const int64_t *__restrict__ tri_indices, const float2 *__restrict__ r1_r2,
                                              ...
                                              float3 *__restrict__ out_normals, float3 *__restrict__ out_colors)
    {
        ...
        sample_points_triangle_mesh_kernel<<<blocks, threads>>>(num_points, vertices, triangles, tri_indices, r1_r2,
                                                                vertex_normals, triangle_normals, vertex_colors,
                                                                out_points, out_normals, out_colors);
    }
     ...
     */
    __global__ void compute_vertex_degree_kernel(const uint32_t num_unique_edges, const int *__restrict__ unique_edges,
     ...
     */
    __host__ void compute_vertex_degree(const uint32_t num_unique_edges, const int *__restrict__ unique_edges,
                                        int *__restrict__ vertex_degrees)
    {
        ...
        compute_vertex_degree_kernel<<<blocks, threads>>>(num_unique_edges, unique_edges, vertex_degrees);
    }
     ...
     * reduction order, and hence the last bits of the result, varies between runs.
     */
    __global__ void compute_uniform_laplacian_kernel(const uint32_t num_unique_edges,
     ...
     */
    __global__ void normalize_uniform_laplacian_kernel(const uint32_t num_vertices,
     ...
     */
    __host__ void compute_uniform_laplacian(const uint32_t num_vertices, const uint32_t num_unique_edges,
                                            const int *__restrict__ unique_edges,
                                            const int *__restrict__ vertex_degrees, const float3 *__restrict__ vertices,
                                            float3 *__restrict__ vertex_lb_uniform)
    {
        if (num_unique_edges == 0 || num_vertices == 0)
            return;

        int threads = NTHREADS;
        int blocks = (num_unique_edges + threads - 1) / threads;

        compute_uniform_laplacian_kernel<<<blocks, threads>>>(num_unique_edges, unique_edges, vertices,
                                                              vertex_lb_uniform);

        int blocks_vert = (num_vertices + threads - 1) / threads;
        normalize_uniform_laplacian_kernel<<<blocks_vert, threads>>>(num_vertices, vertex_degrees, vertex_lb_uniform);
    }
     ...
     * reduction order, and hence the last bits of the result, varies between runs.
     */
    __global__ void compute_voronoi_areas_kernel(const uint32_t num_triangles, const int3 *__restrict__ triangles,
                                                 const float3 *__restrict__ vertices, float *__restrict__ voronoi_areas)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_triangles)
        {
     ...
     */
    __host__ void compute_voronoi_areas(const uint32_t num_triangles, const int3 *__restrict__ triangles,
                                        const float3 *__restrict__ vertices, float *__restrict__ voronoi_areas)
    {
        if (num_triangles == 0)
            return;
        int threads = NTHREADS;
        int blocks = (num_triangles + threads - 1) / threads;
        compute_voronoi_areas_kernel<<<blocks, threads>>>(num_triangles, triangles, vertices, voronoi_areas);
    }
     ...
     */
    __global__ void normalize_cotangent_laplacian_kernel(const uint32_t num_vertices,
     ...
     * reduction order, and hence the last bits of the result, varies between runs.
     */
    __global__ void compute_incident_angles_kernel(const uint32_t num_triangles, const int3 *__restrict__ triangles,
                                                   const float3 *__restrict__ vertices,
                                             ...
                                             float *__restrict__ gaussian_curvature)
    {
        ...
        int threads = NTHREADS;
        int blocks_tri = (num_triangles + threads - 1) / threads;
        compute_incident_angles_kernel<<<blocks_tri, threads>>>(num_triangles, triangles, vertices, vertex_angle_sum);

        int blocks_vert = (num_vertices + threads - 1) / threads;
        finalize_gaussian_curvature_kernel<<<blocks_vert, threads>>>(num_vertices, voronoi_areas, vertex_angle_sum,
                                                                     gaussian_curvature);
    }
    /**
     ...
     */
    __global__ void find_unvisited_kernel(const int num_triangles, const int *__restrict__ visited,
                                          int *__restrict__ seed, int *__restrict__ found)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_triangles)
            return;
     ...
     */
    __global__ void fix_winding_bfs_kernel(const int num_triangles, int3 *__restrict__ triangles,
                                           const int *__restrict__ v2t_offsets, const int *__restrict__ v2t_counts,
                                           const int *__restrict__ v2t_indices, int *__restrict__ visited,
                                           const int *__restrict__ frontier, const int frontier_size,
                                           int *__restrict__ next_frontier, int *__restrict__ next_frontier_size)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            for (int k = 0; k < num_shared; ++k)
            {
                ...
                if (neighbor_id != tri_id)
                {
                    if (atomicCAS(&visited[neighbor_id], 0, 1) == 0)
                    {
                        ...
                        bool n_has_v0_v1 = (n_tri.x == v0 && n_tri.y == v1) || (n_tri.y == v0 && n_tri.z == v1) ||
     ...
     * reduction order, and hence the last bits of the result, varies between runs.
     */
    __global__ void component_signed_volume_kernel(const int num_component_faces,
                                                   const int *__restrict__ component_faces,
                                                   const float3 *__restrict__ vertices,
                                                   const int3 *__restrict__ triangles, float *__restrict__ volumes)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_component_faces)
            return;
     ...
     * @param[in,out] triangles Device array of triangle vertex indices, flipped in place.
     */
    __global__ void invert_component_kernel(const int num_component_faces, const int *__restrict__ component_faces,
     ...
     */
    __host__ void fix_normals(const uint32_t num_triangles, const float3 *__restrict__ vertices,
                              const torch::Tensor &v2t_offsets, const torch::Tensor &v2t_counts,
                              const torch::Tensor &v2t_indices, int3 *__restrict__ triangles)
    {
        ...
        while (true)
        {
            ...
            find_unvisited_kernel<<<blocks, NTHREADS, 0, at::cuda::getCurrentCUDAStream()>>>(num_triangles, d_visited,
                                                                                             d_seed, d_found);
            ...
            int h_one = 1;
            cudaMemcpyAsync(&d_visited[h_seed], &h_one, sizeof(int), cudaMemcpyHostToDevice,
                            at::cuda::getCurrentCUDAStream());
            cudaMemcpyAsync(&d_frontier[0], &h_seed, sizeof(int), cudaMemcpyHostToDevice,
                            at::cuda::getCurrentCUDAStream());
            ...
            {
                cudaMemcpyAsync(&d_component_faces[component_size], d_frontier, frontier_size * sizeof(int),
                ...
                fix_winding_bfs_kernel<<<bfs_blocks, NTHREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
                    num_triangles, triangles, v2t_offsets.data_ptr<int>(), v2t_counts.data_ptr<int>(),
                    v2t_indices.data_ptr<int>(), d_visited, d_frontier, frontier_size, d_next_frontier,
                    d_next_frontier_size);

                cudaMemcpyAsync(&frontier_size, d_next_frontier_size, sizeof(int), cudaMemcpyDeviceToHost,
                                at::cuda::getCurrentCUDAStream());
                c10::cuda::getCurrentCUDAStream().synchronize();
