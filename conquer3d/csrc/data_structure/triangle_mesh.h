...
protected:
    ...
    std::optional<int> vertex_normals_mode =
        std::nullopt;             ///< Weighting used for the cached vertex normals: uniform, area, or angle.
    ...
    std::optional<std::vector<float>> flood_grid_min =
        std::nullopt; ///< Lower corner of the grid the flood fill was computed on.
    std::optional<std::vector<float>> flood_grid_max =
        std::nullopt; ///< Upper corner of the grid the flood fill was computed on.
    std::optional<std::vector<int64_t>> flood_grid_res =
        std::nullopt; ///< Resolution of the grid the flood fill was computed on.

    std::optional<torch::Tensor> cf_coarse_mask =
        std::nullopt; ///< Coarse-level labels from the coarse-fine flood fill.
    std::optional<torch::Tensor> cf_boundary_lookup =
        std::nullopt; ///< Dense coarse-index to boundary-slot lookup, -1 outside the boundary set.
...
public:
     ...
     */
    TriangleMesh(const torch::Tensor &in_vertices, const torch::Tensor &in_triangles,
     ...
     */
    void build_flood_fill_data(std::optional<std::vector<float>> grid_min = std::nullopt,
                               std::optional<std::vector<float>> grid_max = std::nullopt,
                               std::optional<std::vector<int64_t>> res = std::nullopt, int connectivity = 6);
    /**
     ...
     */
    void build_flood_fill_cf_data(std::optional<std::vector<float>> grid_min = std::nullopt,
                                  std::optional<std::vector<float>> grid_max = std::nullopt,
                                  std::optional<std::vector<int64_t>> res = std::nullopt,
                                  std::optional<std::vector<int64_t>> block_size = std::nullopt, int connectivity = 6);
    /**
    ...
    query_points(const torch::Tensor &query_pts, bool return_sdf = false, bool return_prj_pts = true, int sign_mode = 0,
                 int distance_mode = 0, bool return_occ = false);

    /**
     * @brief Accelerated Ray-Mesh intersection queries (Möller-Trumbore).
     */
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
    get_ray_intersection(const torch::Tensor &ray_origins, const torch::Tensor &ray_dirs, bool return_distance = false);
    ...
    sample_points(int num_points, bool uniform = false, bool return_normals = false, bool return_colors = false,
...
namespace triangle_mesh
{
    ...
    __host__ void compute_vertex_normals(const uint32_t num_vertices, const uint32_t num_triangles,
                                         ...
                                         float3 *__restrict__ vertex_normals, int mode = 0);
    ...
    __host__ void build_vertices_to_triangle_map(const uint32_t num_vertices, const uint32_t num_triangles,
