...
namespace kdtree
{
    ...
    struct ZipCompare
    {
         ...
         */
        __device__ bool operator()(const thrust::tuple<uint32_t, float3, int64_t> &a,
    ...
    void build(const uint32_t num_points, float3 *__restrict__ points, int64_t *__restrict__ original_inds)
    {
        ...
        for (int l = 0; l < deepestLevel; l++)
        {
            ...
            update_tags<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(thrust::raw_pointer_cast(tags.data()),
                                                                                  num_points, l);
        }
     ...
     * different branches.
     */
    __global__ void query_kdtree_kernel(const uint32_t num_queries, const uint32_t num_points, const uint32_t k,
                                        const float3 *__restrict__ query_points, const float3 *__restrict__ tree_points,
                                        const int64_t *__restrict__ tree_inds, float *__restrict__ out_dists,
...
#pragma unroll
        ...
        query_kdtree_loop(qp, num_points, tree_points, tree_inds, k, best_dists, best_inds);
               ...
               const int64_t *__restrict__ tree_inds, float *__restrict__ out_dists, int64_t *__restrict__ out_inds)
    {
        ...
        query_kdtree_kernel<<<blocks, threads>>>(num_queries, num_points, k, query_points, tree_points, tree_inds,
                                                 out_dists, out_inds);
    }
} // namespace kdtree