...
namespace bvh
{
     ...
     */
    __device__ __forceinline__ int common_prefix(const int num_objects, const uint32_t *morton_codes, const int i,
                                                 const int j)
    {
        // Out of bounds check: if j is outside the array, they share no prefix (-1)
        if (j < 0 || j >= num_objects)
            return -1;
     ...
     */
    __global__ void compute_morton_codes_kernel(const uint32_t num_objects, const float3 *__restrict__ aabb_mins,
                                                const float3 *__restrict__ aabb_maxs, const float3 scene_min,
                                                const float3 scene_max, uint32_t *__restrict__ morton_codes,
                                                int *__restrict__ object_ids)
    {
        ...
        float3 normalized = make_float3((centroid.x - scene_min.x) / extents.x, (centroid.y - scene_min.y) / extents.y,
     ...
     */
    __global__ void karras_emit_hierarchy_kernel(const int num_objects, const uint32_t *__restrict__ morton_codes,
                                                 int2 *__restrict__ bvh_children, int *__restrict__ bvh_parents)
    {
     ...
     */
    __global__ void bottom_up_aabb_kernel(const int num_objects, const int *__restrict__ object_ids,
                                          const float3 *__restrict__ in_aabb_mins,
                                          const float3 *__restrict__ in_aabb_maxs, const int *__restrict__ bvh_parents,
                                          const int2 *__restrict__ bvh_children, float3 *__restrict__ bvh_aabb_mins,
                                          float3 *__restrict__ bvh_aabb_maxs, int *__restrict__ atomic_flags)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        while (current_node != -1)
        {
            ...
            aabb::compute_aabb_union(bvh_aabb_mins[left_child], bvh_aabb_maxs[left_child], bvh_aabb_mins[right_child],
                                     bvh_aabb_maxs[right_child], bvh_aabb_mins[current_node],
                                     bvh_aabb_maxs[current_node]);
    ...
    void build(const uint32_t num_objects, const uint32_t num_nodes, const float3 *__restrict__ in_aabb_mins,
               const float3 *__restrict__ in_aabb_maxs, float3 *__restrict__ bvh_aabb_mins,
               float3 *__restrict__ bvh_aabb_maxs, int2 *__restrict__ bvh_children, int *__restrict__ bvh_parents,
               int *__restrict__ object_ids)
    {
        ...
        Node scene_bounds = thrust::transform_reduce(policy, iter, iter + num_objects,
                                                     Transform(in_aabb_mins, in_aabb_maxs), init_node, Reduce());
        ...
        compute_morton_codes_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            num_objects, in_aabb_mins, in_aabb_maxs, scene_bounds.min_pt, scene_bounds.max_pt,
            thrust::raw_pointer_cast(morton_codes.data()), object_ids);

        thrust::sort_by_key(policy, morton_codes.begin(), morton_codes.end(), thrust::device_pointer_cast(object_ids));
        ...
        karras_emit_hierarchy_kernel<<<internal_blocks, internal_threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            num_objects, thrust::raw_pointer_cast(morton_codes.data()), bvh_children, bvh_parents);
        ...
        bottom_up_aabb_kernel<<<leaf_blocks, leaf_threads>>>(num_objects, object_ids, in_aabb_mins, in_aabb_maxs,
                                                             bvh_parents, bvh_children, bvh_aabb_mins, bvh_aabb_maxs,
     ...
     */
    __global__ void query_bvh_kernel(const uint32_t num_queries, const uint32_t num_objects,
                                     const float3 *__restrict__ query_mins, const float3 *__restrict__ query_maxs,
                                     ...
                                     int64_t *__restrict__ hit_counter, const int64_t max_capacity)
    {
    ...
    void query(const uint32_t num_queries, const uint32_t num_objects, const float3 *__restrict__ query_mins,
               const float3 *__restrict__ query_maxs, const float3 *__restrict__ bvh_aabb_mins,
               const float3 *__restrict__ bvh_aabb_maxs, const int2 *__restrict__ bvh_children,
               const int *__restrict__ object_ids, int64_t *__restrict__ out_query_ids,
               int64_t *__restrict__ out_object_ids, int64_t *__restrict__ hit_counter, const int64_t max_capacity)
    {
        uint32_t threads = NTHREADS;
        uint32_t blocks = (num_queries + threads - 1) / threads;

        query_bvh_kernel<<<blocks, threads>>>(num_queries, num_objects, query_mins, query_maxs, bvh_aabb_mins,
                                              bvh_aabb_maxs, bvh_children, object_ids, out_query_ids, out_object_ids,
                                              hit_counter, max_capacity);
    }
     ...
     * reached; compare the final counter against it to detect truncation.
     */
    __global__ void query_self_bvh_kernel(const uint32_t num_objects, const float3 *__restrict__ bvh_aabb_mins,
    ...
    void query_self(const uint32_t num_objects, const float3 *__restrict__ bvh_aabb_mins,
                    const float3 *__restrict__ bvh_aabb_maxs, const int2 *__restrict__ bvh_children,
                    const int *__restrict__ object_ids, int64_t *__restrict__ out_query_ids,
                    int64_t *__restrict__ out_object_ids, int64_t *__restrict__ hit_counter, const int64_t max_capacity)
    {
        ...
        query_self_bvh_kernel<<<blocks, threads>>>(num_objects, bvh_aabb_mins, bvh_aabb_maxs, bvh_children, object_ids,
                                                   out_query_ids, out_object_ids, hit_counter, max_capacity);
    }
     ...
     * reached; compare the final counter against it to detect truncation.
     */
    __global__ void query_ray_bvh_kernel(const uint32_t num_queries, const uint32_t num_objects,
                                         const float3 *__restrict__ ray_origins, const float3 *__restrict__ ray_dirs,
                                         ...
                                         int64_t *__restrict__ hit_counter, const int64_t max_capacity)
    {
        ...
        bvh::traverse(
            ...
            {
                float t_hit;
                return ray.is_intersect_aabb(bvh_aabb_mins[node_idx], bvh_aabb_maxs[node_idx], t_hit);
            },
            [&](int leaf_idx)
            {
                uint64_t write_idx = (uint64_t)atomicAdd((unsigned long long int *)hit_counter, 1ULL);
    ...
    void query_ray(const uint32_t num_queries, const uint32_t num_objects, const float3 *__restrict__ ray_origins,
                   const float3 *__restrict__ ray_dirs, const float3 *__restrict__ bvh_aabb_mins,
                   const float3 *__restrict__ bvh_aabb_maxs, const int2 *__restrict__ bvh_children,
                   const int *__restrict__ object_ids, int64_t *__restrict__ out_query_ids,
                   int64_t *__restrict__ out_object_ids, int64_t *__restrict__ hit_counter, const int64_t max_capacity)
    {
        ...
        query_ray_bvh_kernel<<<blocks, threads>>>(num_queries, num_objects, ray_origins, ray_dirs, bvh_aabb_mins,
                                                  bvh_aabb_maxs, bvh_children, object_ids, out_query_ids,
                                                  out_object_ids, hit_counter, max_capacity);
    }
     ...
     */
    __global__ void query_point_bvh_kernel(const uint32_t num_queries, const uint32_t num_objects,
    ...
    void query_point(const uint32_t num_queries, const uint32_t num_objects, const float3 *__restrict__ query_points,
                     const float3 *__restrict__ bvh_aabb_mins, const float3 *__restrict__ bvh_aabb_maxs,
                     const int2 *__restrict__ bvh_children, const int *__restrict__ object_ids,
                     int64_t *__restrict__ out_query_ids, int64_t *__restrict__ out_object_ids,
                     float *__restrict__ out_distances)
    {
        ...
        query_point_bvh_kernel<<<blocks, threads>>>(num_queries, num_objects, query_points, bvh_aabb_mins,
                                                    bvh_aabb_maxs, bvh_children, object_ids, out_query_ids,
                                                    out_object_ids, out_distances);
    }
} // namespace bvh