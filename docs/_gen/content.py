...
#: (file, title, kicker, prose, lead). Every image is real library output,
#: regenerated by docs/_figures/make_figures.py.
FIGURES = [
    (
        "fig-pipeline", "Extraction pipeline", "Pipeline",
    ...
    (
        "fig-normal-modes", "Normal modes", "Normals",
...
FIGURE_SECTIONS = {
    ...
    "isosurface": ("fig-pipeline", "fig-algorithms", "fig-hermite", "fig-resolution",
                   ...
                   "fig-diffrender"),
    "others": ("fig-zcurve", "fig-kdtree", "fig-superquadrics", "fig-sqfit"),
}

#: slug -> (blurb, cover). Covers are chosen for what reads at thumbnail size, and
#: to keep the landing page from loading the heaviest images (fig-diffrender and
#: fig-sign-modes, both over 800 KB) just to show a card.
SECTION_META = {
    "mesh": ("Ray queries, curvature, quality metrics and orientation repair on "
             "triangle meshes.", "fig-curvature"),
    "isosurface": ("From a signed distance field to a surface: extraction, sharp features, "
                   "resolution, normals and differentiable rendering.", "fig-algorithms"),
    "others": ("Spatial ordering, nearest-neighbour search and learnable superquadric "
               "primitives.", "fig-superquadrics"),
}


def _check_sections() -> None:
    """Fail the build when the galleries and the figure list disagree.

    A figure missing from FIGURE_SECTIONS would otherwise vanish from the site
    without a trace, and a slug the nav does not list would produce a page that
    no link reaches.
    """
    slugs = sorted(slug for slug, _ in render.SHOWCASE_SECTIONS)
    if sorted(FIGURE_SECTIONS) != slugs or sorted(SECTION_META) != slugs:
        raise ValueError(
            f"showcase sections {sorted(FIGURE_SECTIONS)} and their metadata "
            f"{sorted(SECTION_META)} must match render.SHOWCASE_SECTIONS {slugs}")
    known = [f[0] for f in FIGURES]
    placed = [name for names in FIGURE_SECTIONS.values() for name in names]
    missing = sorted(set(known) - set(placed))
    unknown = sorted(set(placed) - set(known))
    twice = sorted({name for name in placed if placed.count(name) > 1})
    if missing or unknown or twice:
        raise ValueError(
            "FIGURE_SECTIONS disagrees with FIGURES: "
            f"unassigned {missing or 'none'}; unknown {unknown or 'none'}; "
            f"assigned twice {twice or 'none'}")
    for slug, (_blurb, cover) in SECTION_META.items():
        if cover not in FIGURE_SECTIONS[slug]:
            raise ValueError(f"cover {cover!r} of section {slug!r} is not one of its figures")
...
def _gallery_grid(names) -> str:
    """One gallery's figure cards, lead comparisons first, otherwise in FIGURES order.
    ...
    """
    chosen = [f for f in FIGURES if f[0] in names]
    lead = "".join(_figure_card(f) for f in chosen if f[4])
    rest = "".join(_figure_card(f) for f in chosen if not f[4])
    return f'<div class="fig-gallery">{lead}{rest}</div>'


def showcase_section(slug: str) -> str:
    """One Showcase gallery page: the page header, then its figures."""
    label = dict(render.SHOWCASE_SECTIONS)[slug]
    blurb, _cover = SECTION_META[slug]
    count = len(FIGURE_SECTIONS[slug])
    siblings = "".join(
        f'<a class="btn" href="showcase-{other}.html">{esc(other_label)} →</a>'
        for other, other_label in render.SHOWCASE_SECTIONS if other != slug
    )
    return f"""
<div class="wrap" style="padding-top:52px">
<div class="page-head">
  <div class="crumbs"><a href="index.html">Home</a> › <a href="index.html#showcase">Showcase</a> › {esc(label)}</div>
  <h1 style="font-family:var(--sans)">{esc(label)}</h1>
  <p class="sub">{md_inline(blurb)} {count} figures.</p>
  <div class="btn-row" style="margin-top:18px">{siblings}</div>
</div>
</div>
<section class="section wrap" style="padding-top:8px;border-bottom:none">
  {_gallery_grid(FIGURE_SECTIONS[slug])}
</section>
"""


def _section_card(slug: str, label: str) -> str:
    """A landing-page link into one gallery.

    A plain link styled as a card, not a figure: site.js zooms any .fig-media on
    click, which would open an image here instead of the gallery.
    """
    blurb, cover = SECTION_META[slug]
    count = len(FIGURE_SECTIONS[slug])
    return (
        f'<a class="card section-card" href="showcase-{slug}.html">'
        f'<div class="section-card-media">'
        f'<img src="{img_src(cover)}" alt="" loading="lazy" decoding="async"></div>'
        f'<div class="section-card-body">'
        f'<span class="fig-kicker">{count} figures</span>'
        f"<h3>{esc(label)}</h3><p>{md_inline(blurb)}</p>"
        f'<span class="section-card-go">View gallery →</span>'
        f"</div></a>"
    )


def showcase(version: str, stats: Dict[str, int]) -> str:
    """The landing page: a card for each Showcase gallery, then how to start.

    The figures live on the gallery pages. The cards are also the way into them on
    small screens, where the nav hides its tabs.
    """
    cards = "".join(_section_card(slug, label) for slug, label in render.SHOWCASE_SECTIONS)
    return f"""
<section class="section wrap" id="showcase">
  <div class="section-head">
    <span class="kicker">Showcase</span>
    <h2>Real library output, by topic</h2>
  </div>
...
<section class="section wrap" style="border-bottom:none">
  ...
  <div class="grid grid-2">
    <div>{code_block("pip install -U conquer3d", "bash")}
      ...
      release for Python 3.10–3.14 against PyTorch 2.8 and 2.11.</p>
...
<section class="hero">
  ...
  <div class="hero-inner">
    <div class="hero-copy">
      ...
      <h1>Conquer<span class="grad">3D</span></h1>
