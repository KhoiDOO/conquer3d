...
private:
    ...
    // Size: [2N - 1, 2] -> x = left_child, y = right_child
    torch::Tensor bvh_children;

    // Size: [2N - 1] -> index of parent node
    torch::Tensor bvh_parents;

    // Size: [N] -> Maps the sorted leaf index back to the original Gaussian index
